...
* myproject/
  ...
  + `settings.py` 
    - конфиг проекта Django на высоком уровне (middleware, установленными приложения, роутинг, шаблоны, локаль, базы данных, приложения, статика, базу данных, приложения, ключи, переменные окружения, параметры безопасности, ...)
    ...
    - `CHANNEL_LAYERS` для Django Channels, Redis = бекенд для Pub/Sub и WebSocket-сессий (чат, игра в реальном времени, ...)
...
* на место .app подставляется div
  ...
  + class Component базовый, аострактный - **понять**
  + render значит вернуть html
  + state = переменные 
  + событие DomContqctLoqded - как только загрузился html полностью (у нас это только 1 раз)
  + фиксированная часть страницы доступна в js
  + % body % кберем, т.к. у нас SPA
  + login ph=ges.js - запрос post к бэку
    - с заголовка запроса каждый раз CSRF токен чтобы знать, что это не юзер с третьего сайта
...
* send.msg
* receive = render msg
* ws как передать, кому пишем сообщение? в запросе
* prepMsg забирает инпут и делает ws запрос
* либо взять список друзей из базы и сделать выпадающие список, либоа поле инпут - кому сообщение
* @login_required только если залогинен
  - если нет токена или он неправильный - не пропустит
* отдельный endpoint проверить, существует ли такой юзер (навеное не нужен)
* fetch() запрос обычный (не ws)
* ws один для всех пользователей для чата
* **AbsTimeUser** (непралвьно написано) наш дкасс наследует
* johnResponse - временный
* фронт обрабатвыает как настоящий
* view.py не html
...
* CSS-OM тоже дереао как DOM
* incognito
* в django новая функция python manage.py statup, migrations
* INSTALLED_APS ... 'chat'
* 1 эндпоинт случает чат
  + в заголовке запроса - кому сообщение
* если сообщение не дошло, то такого пользователя нет
* /admin строено в django
