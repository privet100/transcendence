...
* AUTH_USER_MODEL = 'myapp.UserProfile`,  кастомная модель `myapp.UserProfile` вместо стандартного `User`
  ...
  + какую модель использует django: `docker exec -it backend python manage.py shell`, затем `from django.contrib.auth import get_user_model` `print(get_user_model())`
