...
* кэшировнаие у нас
  ...
  + Кэширование позволяет сохранять результаты частых запросов (сложные вычисления, данные из базы) и извлекать их из Redis, что быстрее, чем заново выполнять запросы или расчёты
...
* ещё для других целей (например, `CHANNEL_LAYERS`)
