...
* vm.overcommit_memory
  ...
  + чтобы избежать ошибки `OOM` при больших `maxmemory`
  ...
  + sysctls: vm.overcommit_memory: "1" в docker-compose.yml
    - может не сработать, если контейнер не имеет необходимых прав (по умолчанию Docker не даёт контейнеру доступ к глобальному kernel-параметру, кроме безопасных сетевых `sysctl`)
    - иногда Docker игнорирует «небезопасные» параметры, если контейнер не в privileged-режиме
  + в Dockerfile: RUN echo "vm.overcommit_memory=1" >> /etc/sysctl.conf
    - в большинстве случаев не будет работать по умолчанию, так как внутри контейнера нет полноценного ядра, а Docker запрещает менять ряд параметров
    - нужно либо запускать контейнер с `--privileged`
   + `cat /proc/sys/net/core/somaxconn` 4096   
    - максимально допустимую длину очереди соединений (backlog) для сокета в режиме прослушивания (listen)
    - сколько новых входящих соединений могут быть поставлены в очередь до того, как ядро начнёт отклонять их
    - внутри контейнера listen(fd, 10000) - очередь будет ограничена 4096 (если это не переопределяется другими настройками)
    - значение берётся у хостового ядра, ведь контейнеры используют общее ядро с хостом
  + **защищён паролем** #see
* проверка 
  + `docker-compose exec redis redis-cli PING` - PONG
  + `docker compose logs redis`
  + добавить `redis-cli` в Dockerfile или docker-compose, если у вас не стандартный образ
