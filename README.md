...
* типы вьюх
  + `View`
    - базовый класс для создания вьюх
    - можешь переопределить методы, такие как `get()`, `post()`, `put()`, ...
  + `TemplateView`
    - для рендеринга шаблонов
    - в основном для статических страниц, когда не нужно взаимодействовать с базой данных
  + `RedirectView`
    - для редиректов с одного URL на другой
  + `ListView` и `DetailView`
    - классы для работы с базой данных, предоставляющие автоматическое отображение списка объектов или одного объекта
  + `CreateView`, `UpdateView`, `DeleteView`
    - классы для создания, обновления и удаления объектов в базе данных
  + `APIView` (для DRF)
    - для создания API
    - более гибкий класс, который предоставляет методы для обработки HTTP-запросов, такие как `get()`, `post()`, ...
  + `ViewSet` (для DRF)
    - еще более высокоуровневая абстракция для создания RESTful API
    - не надо вручную описывать методы (`get()`, `post()`, ...)
    - автоматически генерирует методы на основе действия, например, `list`, `create`, `retrieve`, ...
...
* APIView вспомогательный компонент
  + вьюха
  + базовый класс, предоставляет функциональность для обработки запросов (например, `GET`, `POST`, `PUT`, ...)
  + класс для создания API
  + инструмент для создания AP
  ...
  + class UserProfileView(APIView)
    - чтобы другие сервисы или фронтенд могли получить информацию о пользователе через API
    ...
    - Django Admin, интерфейс для администрирования, не предназначен для работы с API, можно просматривать и редактировать данные профиля пользователя
+ class GameView(APIView)
  + class TournamentView(APIView)
* ViewSets вспомогательный компонент
  + вьюха
  + обертка над APIView
  + упрощает работу с CRUD
  ...
  + особенно полезен для упрощения кода и автоматизации создания стандартных операций CRUD
* Routers вспомогательный компонент #see #question
  ...
  + DRF предоставляет класс **`DefaultRouter`** (**`SimpleRouter`**) для автоматической генерации путей (URL-паттернов) к методам ViewSet (`list`, `retrieve`, `create`, `update`, `destroy`)  
