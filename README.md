...
* два SSL-контура, два набора сертификатов:
  + `etc/nginx/ssl/` для главного домена Transcendence (отдача фронта, проксирование бэкенда)
  ...
  + `certif/` для webhook-сервера
    ...
    - `webhook.csr` (Certificate Signing Request) запрос на подпись сертификата
    - `webhook.crt` сертификат, подписанный центром сертификации (или самоподписанный)
    - `webhook.key`   
    - `openssl.cnf` конфигурация OpenSSL для генерации этих файлов
