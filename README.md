...
* у нас папка backend/webhook содержит механизм приёма и обработки webhook-сигналов от GitHub, чтобы автоматизировать процесс деплоя и обновления кода
  + реализация механизма получения уведомлений (webhook) от GitHub и автоматической перезагрузки приложения при обновлении кода
  ...
  + views.py (функция github_webhook):
  + При получении POST-запроса (когда GitHub шлёт уведомление о push-событии или о «ping») валидация проходит несколько этапов:
...
Определение события (X-GitHub-Event):
Если событие — ping, мы отвечаем Pong!, что нужно при первичной настройке вебхука (GitHub проверяет, жив ли ваш endpoint).
Если событие — push и ветка — main, запускаются команды git pull, а затем docker-compose up --build -d. Таким образом:
Происходит автоматическое обновление кода из репозитория.
Запускается или пересобирается Docker-контейнер (или несколько контейнеров), чтобы задеплоить новую версию приложения.
Зачем это нужно?

Автоматизация CI/CD (Continuous Integration / Continuous Delivery): При каждом push в определённую ветку (чаще всего main или master), сервер сам тянет изменения из репозитория и пересобирает контейнеры, фактически обновляя ваше приложение без ручного вмешательства.
Безопасность: Проверка подписи (SECRET_TOKEN) не даёт сторонним пользователям просто так «дёргать» ваш эндпоинт и запускать произвольные команды на сервере.
Удобство разработки: Вам не нужно заходить на сервер, выполнять git pull и вручную перезапускать сервисы. Всё делается автоматически после пуша в GitHub.
