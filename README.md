...
* Should DJANGO_SETTINGS_MODULE be an environment variable? Why do we need it?
...
* js: нопка "создать турнир" -> вызывает 127.0.0.1/tour -> запускает rest framework
* middlware
  + набор классов
...
* использует сессии или JWT (JSON Web Token) для аутентификации пользователей
  ...
  + JWT: Обычно токен передаётся в заголовке авторизации (Authorization: Bearer <token>) и обрабатывается специальным классом аутентификации.
...
* SessionMiddlewareStack: позволяет работать с данными сессии в WebSocket-сообщениях
...
* `manage.py`
  ...
  + точка входа для выполнения административных задач Django (запуск сервера разработки, миграции базы данных, создание пользователей, ...)
  + `execute_from_command_line(sys.argv)` обрабатывает команды, переданные в `sys.argv` (такие как `runserver`, `migrate`, `createsuperuser`, ...)
  + `os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'myproject.settings')` чтобы Django знал, где искать настройки
