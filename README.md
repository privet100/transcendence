...
| **Тип токена/ключа**       | **Назначение**                                                                                     | **Где хранится**                                           | **Особенности**                                                                                     | **Пример использования**                                                                                                                                       |
|----------------------------|---------------------------------------------------------------------------------------------------|-----------------------------------------------------------|-----------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **CSRF-токен**             | Защита от подделки запросов (CSRF-атак).                                                          | В cookie (по умолчанию `csrftoken`).                      | - Проверяется сервером при POST/PUT запросах.<br>- Хранится в cookie и передаётся в скрытом поле формы или заголовке. | - Защита формы входа.<br>- Обеспечение легитимности запросов от пользователя.                                                                                |
| **Токен авторизации**      | Подтверждение авторизации пользователя.                                                           | В HTTP заголовке, cookie, localStorage или sessionStorage. | - Используется для REST API и WebSocket.<br>- Может быть JWT или иным форматом.                                           | - Авторизация REST API (например, `Authorization: Bearer <token>`).<br>- Авторизация WebSocket-соединения.                                                   |
| **Сессионный ключ**        | Управление пользовательской сессией (stateful-приложения).                                        | В cookie (например, `sessionid`, HttpOnly).               | - Долговременная связь пользователя с сервером.<br>- Подходит для веб-интерфейсов.<br>- Сохраняется на сервере.         | - Отслеживание состояния авторизации в веб-приложении.<br>- Сохранение данных пользователя (корзина, настройки).                                              |
| **WebSocket-токен**        | Авторизация пользователя при установке WebSocket-соединения.                                      | В URL параметре или заголовке WebSocket-запроса.          | - Передаётся при установке соединения.<br>- Необходимо для проверки прав доступа.                                         | - Авторизация чата или потоковой системы (например, `wss://example.com/ws/chat/?token=<your-token>`).                                                         |
| **Email-токен**            | Подтверждение email-адреса, восстановление пароля или одноразовая аутентификация.                 | В URL, отправляемом пользователю.                        | - Одноразовый токен.<br>- Генерируется для конкретного действия.                                                       | - Подтверждение email через ссылку: `https://example.com/verify-email/?token=<your-token>`.                                                                   |
| **API-ключи**              | Идентификация и авторизация сторонних приложений или интеграций.                                  | В HTTP заголовке запроса.                                 | - Передаётся с каждым запросом.<br>- Часто имеет ограниченный доступ к API.                                              | - Интеграция с внешним сервисом или предоставление публичного API (например, `Authorization: Api-Key <your-key>`).                                            |
| **Шифровальные ключи**     | Работа с зашифрованными данными (например, шифрование сообщений).                                 | На сервере (в безопасном хранилище).                      | - Не передаётся клиенту.<br>- Используется для шифрования/дешифрования.                                                  | - Шифрование сообщений чата на стороне сервера.<br>- Защита конфиденциальных данных.                                                                          |
| **OAuth-токены**           | Аутентификация и авторизация через внешние сервисы (Google, Facebook, GitHub).                    | В cookie или localStorage.                                | - Используется для работы с OAuth 2.0.<br>- Подтверждает доступ пользователя к сервису.                                  | - Авторизация через Google OAuth API.<br>- Доступ к данным пользователя через API (например, получение email с разрешения пользователя).                     |
| **SSL-сертификат**         | Шифрование трафика между браузером и сервером (HTTPS).                                            | На сервере, используется веб-сервером (например, Nginx).  | - Не является пользовательским токеном.<br>- Обеспечивает безопасность передачи данных.                                  | - Защита пользовательских данных в сети через HTTPS.                                                                                                         |

* **если используется исключительно токен-авторизация (JWT), можно отключить django.contrib.sessions.middleware.SessionMiddleware**
