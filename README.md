...
* WebSocket соединение устанавливается при открытии модального окна и закрывается при его закрытии.
* Комната чата по умолчанию установлена как default_room, но вы можете адаптировать выбор комнаты по вашему усмотрению.
...
+ базовые подходы django
  + FBV function based views
    ...
    - подход более гибкий
    - требует ручной обработки запросов (`GET`, `POST`, ...)
    - проще
    - def my_view(request): return JsonResponse({"message": "Hello, world!"})
  + class based views CBV
    - представления, реализованные как классы, ООП-подход к представлениям 
...
* Django Channels предоставляет инфраструктуру для управления соединениями и маршрутизации сообщений, Channel Layers обеспечивают механизм передачи сообщений между частями приложения
...
* асинхронный обмен сообщениямм, маршрутизация **между**
  ...
  + клиент ↔ consumers: Установление соединения: Клиент инициирует WebSocket-соединение с сервером через потребителя (ChatConsumer или аналогичный)
