...
+ процесс определения личности пользователя (предоставление username, email)  
+ без проверки пароля
+ это не вход в систему  
...
* процесс логина, подтверждение личности, система требует доказательства: пароль, одноразовый код OTP из SMS, отпечаток пальца, лицо, вход через соцсети
...
+ самый простой и безопасный — **cookie**
  - все микросервисы имеют одно и то же имя хоста => файл cookie будет автоматически отправляться браузером при каждом запросе
  - есть расширение для JWT, но мне не удалось заставить его работать с RSA, проще создать собственное промежуточное программное обеспечение для аутентификации
* ТОКЕН АВТОРИЗАЦИИ JWT (основной вариант это JWT)  
  + https://django-rest-framework-simplejwt.readthedocs.io/en/latest/getting_started.html
  + jwt = формат токена для передачи данных между сторонами
  ...
  + для аутентификации в REST API, WebSocket, stateless-приложениях
  + для API и системах с аутентификацией без сохранения состояния (stateless authentication)
  + для API (например, для общения с мобильным приложением или фронтендом
  + содержит информацию о пользователе (например, его ID)
  + самодостаточен
  + без хранения состояния на сервере
  + подписан сервером
  + может храниться в HTTP заголовках, в cookie, в localStorage или sessionStorage
  + nginx хранит jwt не в cookie, а в **локальном хранилище** (в переменной в js, обновляем как только он истечет)
  + клиент после входа получает токен и отправляет его в заголовке `Authorization`, сервер убеждается, что пользователь авторизован
  + с jwt токеном нет необходимости в CSRF-токене
  ...
  + доступность jwt обеспечивается через middleware Channels
  + может быть передан через заголовки WebSocket-сообщений или параметры URL
  + вручную обрабатывать его в consumer
* 42 oauth = разновидность токен-ориентированной аутентификации 
  ...
  + 6) сервер проверяет валидность токена
  + bearer-токен имеет ограниченный срок действия
  + рекомендуется регулярно **обновлять токены через OAuth2 Refresh Token**
...
* СЕССИИОННЫЙ КЛЮЧ = ФАЙЛ СЕССИИ
  ...
  + хранится в cookie `sessionid`
    - cookie `HttpOnly`, чтобы предотвратить доступ к нему из js
    - cookie `Secure`
...
* AUTH_USER_MODEL = 'myapp.UserProfile'
  + django использует кастомную модель `myapp.UserProfile` вместо стандартного `User`
  ...
  + | **Поле**           | **Изменено / Добавлено?** | **Что изменилось?** |
    ...
    | `friends`          | Добавлено                 |  |
    | `picture_url`      | Добавлено                 |  |
    | `avatar`           | Добавлено                 |  |
    | `groups`           | Изменено                  | Добавлен `related_name='userprofile_set'` |
    | `user_permissions` | Изменено                  | Добавлен `related_name='userprofile_set'` |
  + со стандартной моделью django.contrib.auth.models.User без расширения невозможно напрямую добавить аватар, уникальный email, систему друзей, ...
  ...
  + нельзя изменить существующие поля (`username`, `email`)
...
* OAUTH-ТОКЕНЫ
  ...
  + если хранитсяв памяти приложения (In-Memory), в оперативной памяти клиента (например в переменной в js)
