...
* `settings.py` конфиг Django на высоком уровне
  ...
  + `STATIC_ROOT = os.path.join(BASE_DIR, 'staticfiles')`
    ...
    - при запуске `python manage.py collectstatic` -> тут собирвюися все статические файлы приложений, настроенных для работы со статикой 
    - собрать все статические файлы в одном месте для развертывания в продакшн-среде
    - используется при подготовке проекта к продакшн-среде, чтобы собрать все статические файлы в одну папку, которую затем можно обслуживать через сервер
    - **не трекается гитом**
  + `STATIC_URL = '/static/'`
