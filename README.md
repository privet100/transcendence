...
* Пользователь открывает в браузере https://tr.naurzalinov.me/
  ...
  + Запрос попадает на фронтенд-контейнер (Nginx), порт 443
    - Если это запрос к статическому файлу (CSS/JS/картинка HTML, CSS, JS), nginx отдает его из папки /usr/share/nginx/html/static/
    - Иначе (обычный путь /, /api/…, и пр) проксирует запрос на backend:8000, Django обрабатывает API (зависит от nginx.conf)
...
* GitHub (или другой внешний сервис) посылает webhook на https://tr.naurzalinov.me/webhook
  + идёт в отдельный server-блок (Webhook)
  + проксирует запрос на backend:8000/webhook
  + Django обрабатывает пуши GitHub
...
* выполняет бизнес-логику (чат, авторизация, API, управление базой данных, игровая логика и т.д.)
* Взаимодействует с БД и Redis
...
* несколько подпроектов-«приложений» в Django
* api_42, auth_app, chat, module_auth, myapp
...
* staticfiles/ Django или система сборки складывает статические файлы после команды collectstatic
  + обычно такая папка не трекается гитом, но может быть создана для удобства хранения
