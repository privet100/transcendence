...
* можули
* Чаще всего **аутентификация**, **пермиссии** и **сериализация** — это отдельные слои или модули, которые могут работать **как на уровне middleware**, так и непосредственно в вашем коде (например, в контроллерах/вьюшках, сервисах, валидаторах). То есть они **не ограничиваются** только использованием в middleware.

Ниже разберём это на примере типичного веб-фреймворка (Django, NestJS, Express, Flask + DRF и т. п.):

---

### 1. Аутентификация (Authentication)

- **Вариант с middleware**  
  Во многих фреймворках можно подключить middleware, которое проверяет JWT-токен (или сеансовую куку) **до** попадания запроса в код контроллера.  
- **Вариант без middleware**  
  - В Django REST Framework, к примеру, можно указать «классы аутентификации» (authentication classes) в настройках или на уровне отдельных вьюшек. Это работает через механизмы самого DRF, а не напрямую через Django middleware.  
  - В NestJS вы можете прописать «стратегию» в `AuthGuard` и подключать её точечно на конкретных роутах.

Итог: аутентификацию **часто** реализуют как middleware, но в зависимости от фреймворка логика может быть встроена иначе (через декораторы, guards, менеджеры и т. д.).

---

### 2. Пермиссии/права доступа (Permissions)

- **Middleware**  
  Некоторые фреймворки позволяют написать «авторизационное» middleware, которое проверяет роли пользователя.  
- **Методы или декораторы в контроллерах/классах**  
  - Например, в Django REST Framework есть «permission classes», которые вызываются на уровне вью (View) и решают, разрешён ли доступ.  
  - В NestJS есть декораторы вроде `@Roles('admin')`, которые работают совместно с «guards», обрабатывая логику до/вместо контроллера, но чуть более «точечно», чем глобальный middleware.

Итог: разрешения **могут** обрабатываться в middleware, но нередко это делается на уровне самих обработчиков (контроллеров/вьюшек) с помощью встроенных механизмов фреймворка. 
...
- **Часто **не** middleware**  
  Сериализация JSON/XML/… обычно делается либо в слоях, отвечающих за формирование ответа, либо непосредственно в контроллере/вью, либо в специализированном «сериализаторе» (например, Django REST Framework `Serializer`, NestJS `class-transformer`, etc.).  
- **Иногда как middleware**  
  - Вы можете использовать middleware, которое автоматически преобразует `req.body`/`res.body` (или `req` и `res`) в/из JSON. В том же Express есть `express.json()` — формально это middleware.  
  - Но если говорить о «модулях сериализации», которые валидируют поля, фильтруют и конвертируют объекты в правильный формат, то обычно это не совсем «middleware» в классическом смысле, а более специфичный слой фреймворка.

Итог: сериализация может частично работать на уровне middleware (например, `body-parser`), но основная логика сериализации/десериализации/валидации обычно живёт отдельно в специальных классах/модулях, а не внутри типового middleware.

---

## Вывод

- **Аутентификация** и **проверка прав (Permissions)** могут быть как middleware, так и отдельными слоями/декораторами/guards.  
- **Сериализация** нередко реализуется вне middleware — в движке фреймворка (например, сериализаторы в DRF, DTO в NestJS) или в отдельных библиотеках.

Поэтому короткий ответ: 
> «**Они не только для middleware.** Аутентификация и permissions могут быть частью middleware, но также могут работать на уровне самих вью/контроллеров. Сериализация (особенно бизнес-логика сериализаторов) обычно реализуется отдельно, а middleware часто занимается лишь базовым парсингом JSON из body.»
> 
