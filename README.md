...
* django:
  + бэкенд
  + со всеми ключевыми механиками (аутентификация, управление базой данных, админка, API), множество готовых функций и лучших практик прямо из коробки
    - что накладывает свои рамки в архитектуре
    - диктует архитектуру (приложения, модели, views, urls и т.д.), что помогает поддерживать чистоту кода и масштабируемость
  + фронт работает через HTTP/WS-протоколы
  + фронт использует эндпоинты Django для обмена данными и интерактивного взаимодействия (чат, игра, профили пользователей)
...
* myapp
  ...
  + модель `UserProfile`: инфо о пользователе, его дружеских связях (many-to-many на самого себя), аватаре, дате последней активности и т. д
  ...
  + модель `Game` хранит историю сыгранных матчей, указывает, кто участвовал, какой турнир (если есть), счёт и определяет победителя
    - хранит результаты матча между двумя пользователями, дату, принадлежность к какому-то турниру
  + `friends = models.ManyToManyField("self")`: пользователи могут быть друзьями 
  + если нужно расширить профили (добавить рейтинг, биографию, статистику), турнирную логику (сетка турнира, раунды) или механику игр (несколько разных типов игр), проще всего добавлять поля и миграции в соответствующие модели в этом приложении
  + `models.py`  
  + `views.py`  
...
### SSL (HTTPS)
* шифровать соединение между клиентом (браузером пользователя) и сервером
...
* `ssl_ciphers 'HIGH:!aNULL:!MD5';` - можно использовать надежные шифры, `!aNULL` и `!MD5` исключают небезопасные варианты  
* `ssl_prefer_server_ciphers on;` указывает браузерам отдавать предпочтение списку шифров сервера, а не списка клиента
* Настройка `proxy_set_header ...` передает заголовки (Host, IP-адрес клиента, протокол и т.д.), чтобы бэкенд понимал, откуда пришел запрос
* два SSL-контура (основной + webhook), два набора сертификатов:
  + `etc/nginx/ssl/`
    - для главного домена Transcendence (отдача фронта, проксирование бэкенда)
    - certificate.crt, private.key, certificate.csr — основной SSL-сертификат и приватный ключ для сайта (домена, где крутится Transcendence)
    - `nginx.conf: ssl_certificate /etc/nginx/ssl/certificate.crt; ssl_certificate_key /etc/nginx/ssl/private.key;
    - этот набор обеспечивает HTTPS для основного сайта/приложения
  + `certif/`  
    - для webhook-конфигурации
    - webhook.crt, webhook.key, webhook.csr, конфиг `openssl.cnf`  
    - отдельный сертификат (и приватный ключ) для webhook-сервера: запросы от GitHub на `https://…/webhook`, и это делаетесь под каким-то другим доменом или хотите хранить self-signed сертификат в отдельном мест  
    - `.csr` (Certificate Signing Request) — запрос на подпись сертификата
    - `.crt` сам сертификат, подписанный центром сертификации (или самоподписанный)
    - `openssl.cnf` используют при генерации этих файлов (конфигурация OpenSSL)
