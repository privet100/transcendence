...
* middlware
  + стандартные компоненты Django (ядра фреймворка)
  + не часть DRF или каких-то REST‐библиотек
  + набор классов
...
* APIView
  ...
  + позволяет создавать собственные представления для обработки запросов
  + предоставляет методы (`get`, `post`, `put`, `delete`, и т.д.), которые можно переопределять для обработки HTTP-запросов
  ...
  + позволяет применять ограничения на количество запросов.
  + больше контроля, чем ViewSet, самостоятельно определяете логику запросов и маршрутизацию.
