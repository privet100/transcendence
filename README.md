...
### Dockefiles
* `context ./backend`
  + файлы в `./backend` доступны для процесса сборки Docker
  + что отправлять демону для создания образа
  + только файлы COPY, ADD, ... скопируются в контейнер
  + `./backend` рабочая область для команд в Dockerfile
* `WORKDIR /app`
  + определяет рабочую директорию внутри контейнера, где будут выполняться команды
  + стоит использовать, даже если копируете все файлы в корень (`/`)
