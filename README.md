...
* защищает от атак подделки межсайтовых запросов (CSRF-атак)
  ...
  + CSRF-атаки: пользователь вошел в аккаунт на сайте, а затем перешел на вредоносный сайт, сайт отправиляет запрос от имени пользователя
  + prevents a POST request from subdomain.example.com from succeeding against api.example.com
* в stateful-приложениях
  + где sessionid или cookie для отслеживания пользователя
...
1) первый запрос клиента, js запрашивает CSRF-токен с эндпоинта `api/csrf-token/`
2) django создаёт токен (заново для каждой сессии)
2) django сохраняет токен в cookie `csrftoken` 
  + с клиентской стороны только для чтения (не `HttpOnly`)
  + не доступен для сторонних доменов (`SameSite=Lax` или `SameSite=Strict`)
  + `Secure`
4) отдаёт клиенту в формате JSON
5) js сохраняет токен в cookie `document.cookie = csrftoken=...`
1) второй запрос клиента, токен в заголовке `POST`, `PUT`, `DELETE`-запросов или скрытом поле формы при post/get запросах
