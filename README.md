...
* Пользователь открывает в браузере https://tr.naurzalinov.me/
  ...
  + Запрос попадает на фронтенд-контейнер (Nginx), порт 443
    - если это запрос к статическому файлу (CSS/JS/картинк/HTML), nginx отдает его из /usr/share/nginx/html/static/
...
* Nginx — серверный компонент, кажется логичным хранить его конфигурацию в папке, связанной с бэкендом. Но тут `nginx.conf` лежит в папке `frontend` по следующим причинам:
  ...
  + Проксирование бэкенда  
   ...
   - Несмотря на то, что Nginx выполняет «серверную» задачу проксирования, он всё равно обслуживает именно фронтенд-контейнер. Поэтому разработчики нередко кладут `nginx.conf` в папку `frontend/`, подчёркивая, что это часть «внешнего (front)» эшелона приложения.
  ...
  + `nginx.conf` лежит в папке `frontend/` не потому, что Nginx — «клиентская технология», а потому, что в Docker-архитектуре этот сервис называется `frontend` и отвечает за раздачу статики и проксирование запросов к бэкенду.  
