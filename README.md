...
* случайная строка
* что защищает:
  + validation for forms and any user input, формы **на странице входа** 
  + UI-приложения
* защищает от атак подделки межсайтовых запросов (CSRF-атак):
  ...
  + для предотвращения подделки запросов от пользователя через его браузер
    - актуально для HTTP-запросов, особенно `POST`, `PUT`, `DELETE`
...
* в SPA-приложениях, где **CSRF-токен не всегда автоматически передаётся через куки** (например, если `HttpOnly` установлен)
...
6) csrf middleware проверяет токен
3) django сравнивает токен из запроса с токеном из cookie
  + чтобы знать, что это не юзер с третьего сайта
  + убедиться, что запрос от легитимного пользователя
4) views.py **набор проверок**
* передаётся в ответах сервера
  + если есть текущий CSRF-токен, привязанный к пользователю (**сессии или куки**), возвращает его  
* after logging in in another browser tab or hitting the back button after a login, you reload the page with the form (the token is rotated after a login)

  + F12 network сокет: accept-encoding:
