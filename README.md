### Ошибка 404 на `/game/1/`, в то время как `/user/1/` загружается с 403
* запусти `runserver`, `http://localhost:8000/game/1/`: Если ошибка 404 сохраняется, значит, Django не знает про этот маршрут
* Проблема с REST Framework и permissions  
  + проверь `APIView`, долдны быть `queryset = Game.objects.all()` и `lookup_field = 'id'`


### **Анализ проблемы:**
1. **Запросы к `localhost:4443/game/1` и `localhost:4443/user/1` действительно отправляются.**  
2. **Django отвечает `200 OK` на `/user/1`, но `404 Not Found` на `/game/1/`.**  
   - Это значит, что эндпоинт `/game/1/` не существует на сервере.  
   - В то же время, `fetch('/game/1')` отвечает `200`, что странно. Возможно, фронтенд подменяет ответ.

---

### **Возможные причины ошибки с `/game/1/`**
#### 🔍 **1. Ошибка в маршрутах Django (`urls.py`)**
Проверь, есть ли в `urls.py` соответствующий эндпоинт:
```python
from django.urls import path
from .views import GameDetailView  # Убедись, что импорт верный

urlpatterns = [
    path('game/<int:pk>/', GameDetailView.as_view(), name='game-detail'),
]
```
✅ **Исправь, если путь `/game/<int:pk>/` отсутствует.**

---

#### 🔍 **2. Ошибка в запросе — `/game/1` vs `/game/1/`**
Обрати внимание, что сервер отвечает **`404 Not Found: /game/1/`**, а запрос отправляется на **`/game/1`** (без `/` в конце).  

📌 **В Django маршруты чувствительны к слэшу**. Попробуй добавить `APPEND_SLASH=True` в `settings.py`:
```python
APPEND_SLASH = True
```
Или измените `urls.py`, добавив `re_path` для обработки без `/`:
```python
from django.urls import path, re_path

urlpatterns = [
    path('game/<int:pk>/', GameDetailView.as_view(), name='game-detail'),
    re_path(r'^game/(?P<pk>\d+)$', GameDetailView.as_view()),  # Обрабатывает без слэша
]
```
✅ **Попробуй после этого снова запросить `/game/1`.**

---

#### 🔍 **3. Фронтенд рендерит страницу вместо запроса?**
Раз `fetch('/game/1')` отвечает `200`, а Django выдаёт `404`, это может значить, что **фронтенд сам подменяет ответ.**  

📌 **Проверь, что `/game/1` не используется в `Router.js` (React, Vue, Vanilla JS) как статическая страница.**  
- В твоём `Router.js` нет `/game/1`, значит, запрос точно должен идти на сервер.  

✅ **Открой в DevTools вкладку "Network", кликни на запрос `/game/1` и проверь "Response" — какой JSON приходит от сервера?**

---

### **Что делать дальше?**
1. **Проверь `urls.py` — есть ли `/game/<int:pk>/`?**
2. **Попробуй добавить `APPEND_SLASH=True` или `re_path` для `/game/1`.**
3. **Посмотри, что возвращает сервер в "Network → Response" на `/game/1`.**
4. **Запроси вручную `fetch('/game/1/')` (с `/` в конце) и посмотри, работает ли.**

Если проблема не решается, напиши мне вывод после этих шагов! 🔍


Судя по информации, которую ты предоставил, запросы к `/user/1` и `/game/1` действительно отправляются на сервер. Вот несколько моментов, которые важно учитывать для диагностики проблемы:

---

### **1. Для запроса `/user/1` все в порядке**
- Ответ сервера **200 OK** подтверждает, что запрос к `/user/1` успешный.
- Страница для этого URL рендерится, и сервер отвечает без ошибок.

---

### **2. Для запроса `/game/1` проблема:**
- **Ошибка 404 (Not Found)** указывает, что сервер не может найти путь `/game/1/`.
- Логи показывают, что сервер не может найти этот маршрут и выдает **404**, что означает отсутствие обработчика для этого URL в Django.

### **Что нужно проверить:**

1. **URL в Django**:
   Убедись, что ты правильно настроил маршрут в Django для `/game/1`. Возможно, этот путь не прописан в файле `urls.py` (или в другом файле с маршрутизацией).
   ...
   Важно, чтобы в `views.py` был определён метод `game_detail`, который будет обрабатывать запрос.
...
2. **Наличие правильного слэша в URL**:
   В твоих логах есть ошибка для `/game/1/`, но ты вызываешь `/game/1`. Возможно, сервер ожидает путь с дополнительным слэшем на конце, или наоборот, путь должен быть без слэша.
   - Убедись, что в конфигурации маршрутов у тебя или есть `/game/<int:game_id>/` или `/game/<int:game_id>` в зависимости от предпочтений (и они совпадают с тем, что ожидается на фронтенде).

3. **Проверь настройки на фронтенде**:
   Если ты отправляешь запрос на `/game/1` на фронтенде, убедись, что URL соответствует тому, что ожидает сервер. Например, если сервер ожидает `/game/1/`, а фронтенд отправляет `/game/1`, то может возникнуть 404 ошибка.
   
   Ты можешь попробовать изменить путь в `Router.js`:
   ```js
   '/game/1/': () => new GamePage().render(),  // с добавлением слэша в конце
   ```

---

### **Шаги для решения проблемы**:
1. **Проверь путь в Django (urls.py)**: Убедись, что в `urls.py` правильно настроен маршрут, который соответствует `/game/1/` или `/game/1`, в зависимости от того, что ты используешь.
2. **Проверь фронтенд (Router.js)**: Убедись, что запрос на фронтенде отправляется правильно на тот же маршрут, что и на сервере (с учётом слэша или без).
3. **Проверь обработчик запроса** в Django (`views.py`), чтобы убедиться, что он существует и правильно обрабатывает запрос.

После этого сервер должен начать обрабатывать запросы к `/game/1` без ошибок 404.
...
* ТОКЕН АВТОРИЗАЦИИ (JWT, Bearer-токены, ...)  
...
* сессия
  ...
  + после успешного входа сервер создает сессию, которая привязывается к пользователю
  + в дальнейшем эта сессия подтверждает аутентификацию, так что пользователь не должен вводить пароль при каждом запросе
* 42 oauth
...
* AUTH_USER_MODEL = 'myapp.UserProfile'
  ...
  + какую модель использует django?
    ...
    - затем в интерактивном режиме:  
      ...
      ```
  + после изменения `AUTH_USER_MODEL` делать миграции
  + если другие модели ссылаются на `User`, они должны использовать `get_user_model()`
  + | **Поле** | **Изменено / Добавлено?** | **Что изменилось?** |
    |----------|-----------------|----------------|
    | `username` | Изменено | `max_length=200` вместо `150` |
    | `email` | Изменено | Сделан обязательным и уникальным |
    | `friends` | Добавлено | Реализована система друзей (связь многие-ко-многим) |
    | `picture_url` | Добавлено | URL для фото пользователя |
    | `avatar` | Добавлено | Поле для загрузки изображения |
    | `groups` | Изменено | Добавлен `related_name='userprofile_set'` |
    | `user_permissions` | Изменено | Добавлен `related_name='userprofile_set'` |
  + со стандартной моделью `User` без расширения невозможно напрямую добавить аватар, уникальный email, систему друзей, ...
  + | **Функциональность** | **Возможно в стандартной `User`?** | **Как реализовать без расширения?** |
    |----------------------|---------------------------------|---------------------------------|
    | **Аватар (`avatar`)** | ❌ Нет | Создать **отдельную модель `UserProfile`**, связанную с `User` через `OneToOneField` |
    | **Система друзей (`friends`)** | ❌ Нет | Создать модель **Friendship**, связанную `ManyToManyField(User)` |
    | **Уникальный `email`** | ❌ Нет (по умолчанию) | Переопределить `UserManager`, сделать `email` `unique=True` |
    | **URL для фото (`picture_url`)** | ❌ Нет | Добавить поле в `UserProfile` |
    | **Изменённый `username`** (до 200 символов) | ❌ Нет (`max_length=150`) | Переопределить модель, изменить `max_length` |
    | **Свои `groups` и `permissions`** | 🔸 Частично | Можно настроить, но `related_name` нельзя изменить без расширения |
  + можно добавить многие фичи без расширения через связанные модели (`UserProfile`, `Friendship`)  
  + нельзя изменить существующие поля** (`username`, `email`) без расширения `User`
  + если хочешь глобально изменить пользователя (например, `email` вместо `username`), лучше использовать `AUTH_USER_MODEL`
  + стандартная модель это django.contrib.auth.models.Use
...
**если используется исключительно токен-авторизация (JWT), можно отк
лючить contrib.sessions.middleware.SessionMiddleware и middleware.csrf.CsrfViewMiddleware**
* WEBSOCKET TOKEN
  + для авторизации пользователя при установке ws-соединения
  + Проверка прав доступа и идентификация пользователя
  + при установке ws-соединений клиент передаёт токен в URL или заголовках
    - `wss://example.com/ws/chat/?token=<your-token>`.
  + могут быть основаны на JWT или другом формате, но передача должна быть защищена (например, через HTTPS/WSS)
* OAUTH-ТОКЕНЫ
  + если хранится на стороне клиента в Local Storage
    - простота реализации
    - данные остаются после перезагрузки страницы.
    - злоумышленники могут украсть токен, если внедрят вредоносный js-код (XSS-атака)
  + если хранится на стороне клиента в Session Storage
    - простота реализации
    - безопасность: данные удаляются при закрытии вкладки
  ...
  + если хранится на стороне сервера в базе данных (**Access Token, Refresh Token**)
    ...
    - ограничить доступ к таблице с токенами только необходимыми сервисами
  + если хранится на стороне сервера в кэше Redis 
    - Быстрый доступ
    - Легко управлять истечением токенов (**TTL**)
    - данные в Redis очищаются при сбое или перезагрузке
  ...
  + backend использкет токены для взаимодействия с внешними API без необходимости передачи токенов на Frontend
  + получение Токенов
...
* в REDIRECT_URI_42=http://127.0.0.1:8000/auth/callback надо **убрать 127.0.0.1:8000**
