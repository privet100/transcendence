...
* запусти `runserver`, `http://localhost:8000/game/1/`: Если 404, то Django не знает про этот маршрут
* Проблема с REST Framework и permissions  
  + проверь `APIView`, долдны быть `queryset = Game.objects.all()` и **`lookup_field = 'id'`**
* эндпоинт `/game/1/` не существует на сервере  
* `fetch('/game/1')` отвечает `200`
  + возможно, фронтенд подменяет ответ
  ...
  + DevTools - Network - кликни на запрос `/game/1`, проверь "Response" — какой JSON приходит от сервера?
* проверь `urls.py`
* APPEND_SLASH = True
* 404 = сервер не может найти путь `/game/1/`
  + что означает отсутствие обработчика для этого URL в Django
