...
#### SSL/TLS 
* не является токеном или ключои в традиционном понимании
...
* 1) Подготовка сертификата и настройка Nginx
  ...
  + отправляем .csr в удостоверяющий центр (.key е отправляем)
  + получаем .crt (публичный ключ сервера + подпись CA)
  + копируем .crt, .key в контейнер Nginx 
    - `.key` хранится на сервере
  + `nginx.conf` настройки шифрования трафика
* 2) Установка защищённого соединения (TLS-рукопожатие)
  + браузер отправляет запрос для установки HTTPS-соединения (ClientHello)
    - запрос содержит поддерживаемые версии TLS, наборы шифров cipher suites, случайные данные
  + nginx отправляет браузеру .crt (ServerHello + Certificate)
  + браузер сверяет .crt с доверенными центрами сертификации (CA)
    - подтверждает подлинность сервера (1-я функция SSL – Аутентификация сервера)
  + браузер генерирует pre-master secret (предварительный ключ для симметричного шифрования)
    - может использоваться для последующих соединений (не обязателен)
  + браузер шифрует pre-master secret с использованием .crt
  + браузер отправляет pre-master secret серверу (ClientKeyExchange)
  + браузер генерирует сессионный симметричный ключ Session ID на основе pre-master secret
    - это ключ для дальнейшего симметричного шифрования данных
  + nginx получает pre-master secret
  + nginx расшифровывает pre-master secret с помощью .key
    - используем .key только тут
  + nginx сохраняет pre-master secret
  + nginx генерирует session id (сессионный симметричный ключ) на основе pre-master secret (такой же ключ, как и сгенерированный браузером?)
...
* случайная строка, уникальная для каждой сессии **или формы**
* нужен:
...
* getCSRFTokenFromCookie() сделать **две одинаковыми** #see
* https://docs.djangoproject.com/en/5.1/howto/csrf/
* 1. JS на фронте читает инпут, проверяет с помощью `regex` - валидация данных (проверка e-mail, пароля, ...) 
  + не имеет отношения к CSRF
* 2. Первый запрос CSRF-токенв:
  + либо django.middleware.csrf.get_token запрашивает  с `api/csrf-token/` или **при рендере HTML-страницы**с
  + либо методы POST/PUT/DELETE при включённой CsrfViewMiddleware 
  + либо @ensure_csrf_cookie
* 3. django либо генерирует токен, лиюо возвращает уже существущий в cookie/сессии
* 4. django отдаёт клиенту токен
  + Либо устанавливает cookie `csrftoken` через **`@ensure_csrf_cookie`** (предпочтительнее)
    - Только для чтения клиентом (не `HttpOnly`)
  ...
  + Либо явно в формате JSON
    - если SPA должен явно его прочитать
    - или если `HttpOnly` установлен
    - **или при загрузке страницы**
    - JS вручную сохраняет его в cookie или локальном хранилище
* 5. второй запрос клиента: js обращается к `/profile`, `/chat/`, ...
  + CSRF-токен в заголовке `X-CSRFToken` для `POST`, `PUT`, `PATCH`, `DELETE` запросов
* 6. `CsrfViewMiddleware` глобальная CSRF-защита
  - проверяет совпадение токена из запроса и из cookie/сессии
  - использует constant-time comparison для предотвращения атак по времени
  - принимает только запрос от авторизованного пользователя, с тем же Orgigin, с того же сайта
  - ту же самую работу может проделать декоратор `@csrf_protect`
  - **Referer** проверяется для запросов по HTTPS, чтобы избежать **атак через смешанный контент**
* 7. `views.py` дополнительные проверки (авторизация, валидацию данных, др)
  + не имеет отношения к CSRF

Когда говорят, что `CsrfViewMiddleware` проверяет совпадение токена из запроса и cookie/сессии, это происходит **внутри самого `CsrfViewMiddleware`** на стороне бэкенда, без использования внешних файлов. Давай разберёмся подробнее:

---

### 🔍 **Где Django хранит CSRF-токен?**  
1. **В cookie (`csrftoken`)**  
   - 📦 Хранится в HTTP cookie, который отправляется клиенту.  
   - Пример:  
     ```http
     Set-Cookie: csrftoken=abc123xyz; Path=/; SameSite=Lax; Secure
     ```  
   - При следующем запросе браузер автоматически отправит этот cookie обратно на сервер:  
     ```http
     Cookie: csrftoken=abc123xyz
     ```

2. **В сессии** (если используется session-based CSRF)  
   - Если Django настроен использовать сессии (`request.session`), токен может быть сохранён там.  
   - Тогда он лежит в памяти или базе данных, которую Django использует для хранения сессий (например, в базе данных `django_session`).

---

### ⚙ **Где и как именно `CsrfViewMiddleware` выполняет проверку?**

1. **Middleware:**  
   - Код `CsrfViewMiddleware` находится в Django (`django.middleware.csrf.CsrfViewMiddleware`).  
   - При каждом запросе, **до вызова view**, middleware автоматически срабатывает.  
   - Он ищет CSRF-токен:  
     - В заголовке запроса (`X-CSRFToken`) или скрытом поле формы.  
     - В cookie `csrftoken`.  
     - В сессии (`request.session`), если используется сессионное хранилище.

2. **Проверка токена:**  
   - Django использует функцию `django.middleware.csrf._does_token_match` для безопасного сравнения токенов (сравнение за константное время, чтобы предотвратить атаки по времени).  
   - ✨ *Важно:* Django **не читает токен из файлов** — всё берётся либо из памяти текущего запроса (`request.COOKIES` и `request.session`), либо из заголовков запроса.

---

### 🔒 **Где это в DRF?**  
- Если ты используешь **Django REST Framework**, CSRF-защита всё равно базируется на стандартном `CsrfViewMiddleware`.  
- В DRF это дополнительно управляется классом:  
  ```python
  from rest_framework.authentication import SessionAuthentication

  class CsrfExemptSessionAuthentication(SessionAuthentication):
      def enforce_csrf(self, request):
          return  # CSRF проверка отключена для этого примера
  ```
- Но по умолчанию DRF также полагается на стандартный механизм Django.

---

### 📝 **Итог:**  
🔹 **Токен из cookie:** `request.COOKIES['csrftoken']`  
🔹 **Токен из запроса:** `request.headers['X-CSRFToken']` или POST-данные формы  
🔹 **Проверка:** внутри `CsrfViewMiddleware` в памяти во время обработки запроса (без записи в файл)  


Да, всё верно! 🔥 Давай разберём шаг за шагом:

---

### ✅ **Как `CsrfViewMiddleware` сравнивает CSRF-токены:**

1. **Клиент отправляет запрос:**  
   - При `POST`, `PUT`, `PATCH`, `DELETE` (и других "изменяющих" запросах) клиент должен отправить CSRF-токен:  
     - 📜 **В заголовке:**  
       ```http
       X-CSRFToken: abc123xyz
       ```  
       *(JS сам добавляет этот заголовок, если ты прописал это явно)*  
     - 💾 **Или в теле формы:**  
       ```html
       <input type="hidden" name="csrfmiddlewaretoken" value="abc123xyz">
       ```

2. **Cookie автоматически добавляется браузером:**  
   - Когда браузер делает запрос к тому же домену, он **автоматически** прикрепляет все подходящие cookie.  
   - В том числе:  
     ```http
     Cookie: csrftoken=abc123xyz
     ```

3. **Middleware проверяет совпадение:**  
   - `CsrfViewMiddleware` на бэкенде сравнивает:  
     - Значение из заголовка (`X-CSRFToken`) или скрытого поля формы  
     - Со значением из cookie (`csrftoken`)  
   - Если они совпадают — запрос проходит дальше.  
   - Если нет — Django возвращает ошибку `403 Forbidden`.

---

### 💡 **Зачем нужен этот механизм?**  
Идея в том, что злоумышленник, даже если обманет пользователя открыть вредоносную страницу, **не сможет получить доступ к токену из cookie**, чтобы добавить его в заголовок. Ведь:  
- 🛡️ Cookie отправляется автоматически браузером.  
- 🛡️ Но заголовок `X-CSRFToken` клиент должен указать **вручную** (JS-код должен его добавить).  
- 🔒 **Другие сайты** не имеют доступа к cookie, не могут прочитать `csrftoken` из браузера пользователя, если настроен `SameSite=Lax` или `Strict`.  

---

### 🎯 **Подытог:**  
- ✅ **Первый токен:** Приходит в заголовке `X-CSRFToken` или теле формы (клиентский JS добавляет его явно).  
- ✅ **Второй токен:** Приходит **автоматически** через cookie `csrftoken` (браузер добавляет при запросе).  
- ✅ **Сравниваются:** В `CsrfViewMiddleware` внутри Django.  



🔐 **Отличный вопрос!** Да, клиент **теоретически** может подделать и заголовок запроса, и cookie, но тут есть ключевой момент: **серверу всё же есть с чем сравнивать.** Давай разберёмся.

---

### 🧩 **Почему CSRF-защита всё же работает, даже если клиент может подделать данные?**

#### 💾 **Где хранится "настоящий" токен у сервера?**
- Django **не просто полагается на cookie**.  
- ✅ **Если используется сессия (`SessionMiddleware`)**, CSRF-токен хранится на стороне сервера — в данных сессии (например, в базе данных или Redis, если так настроено).
- ✅ **Если используется cookie-only защита** (по умолчанию в Django с `CsrfViewMiddleware`), защита основана на том, что:  
  - CSRF-токен — это **криптографически защищённая случайная строка**.  
  - Он **подписан с помощью секретного ключа Django (`SECRET_KEY`)**.  
  - Сервер при проверке делает **криптографическую верификацию** этого токена, используя `SECRET_KEY`.  
  - Клиент **не знает** этот ключ и, следовательно, **не может подделать корректный токен**.

---

### 🔒 **Как именно работает проверка на сервере?**  
1. Клиент присылает CSRF-токен (в заголовке или форме) + автоматически прикладывается cookie.  
2. Django использует функцию вроде `constant_time_compare` для сравнения двух значений.  
3. 🧮 Если используется cookie-only режим, токен в cookie представляет собой что-то вроде:  
   ```
   csrftoken: XoZerE3u4V3g6LhKq9... (сгенерировано с подписью)
   ```
   Сервер проверяет его подпись с помощью `SECRET_KEY`.  
4. Если подпись верна — токен считается аутентичным. Если нет — 403 ошибка.

---

### ⚡ **Почему злоумышленник не может подделать токен?**  
- Чтобы создать действительный CSRF-токен, злоумышленнику нужен **`SECRET_KEY`** приложения Django.  
- Без него токен не пройдет валидацию, даже если клиент "сымитирует" cookie и заголовок.  
- 🔐 **Сравнение происходит не просто по значению токена, а с криптографической проверкой.**

---

### ✋ **Но если токен в cookie и заголовке совпадает, почему это безопасно?**  
Потому что:  
- 🌐 Браузер автоматически добавляет cookie только для исходного домена.  
- 🛡️ Другой сайт не может прочитать cookie из браузера пользователя из-за политики одного источника (Same-Origin Policy).  
- 💣 Даже если злоумышленник может отправить запрос с поддельным заголовком, **он не знает правильного токена**, соответствующего подписи.

---

### 📝 **Итог:**  
- 🛠️ Клиент может подделать данные, но **без знания `SECRET_KEY` подделка CSRF-токена невозможна.**  
- 💎 **Django проверяет подпись токена**, а не просто его совпадение.  
- 🔒 Поэтому у сервера **есть с чем сравнивать**, и CSRF-защита остаётся надёжной.
...
Нет, это **разные классы** в Django REST Framework (DRF), хотя все они связаны.  

### **1️⃣ `BaseRouter`** (Базовый класс)  
- Это **абстрактный** класс, от которого наследуются все остальные роутеры.  
- Он сам **не используется напрямую**.  
- Определяет базовую логику для работы с маршрутами.  
...
class MyCustomRouter(BaseRouter):
    def get_urls(self):
        return []  # Нужно переопределить этот метод
```

---

### **2️⃣ `SimpleRouter`**  
- **Минимальный роутер** с базовым функционалом.  
- **Не добавляет `root-view`** (список всех маршрутов API).  
...
router = SimpleRouter()
router.register(r'users', MyViewSet, basename='user')

urlpatterns = router.urls
```

📌 **Когда использовать?**  
✅ Если не нужен `/api/` со списком всех маршрутов.

---

### **3️⃣ `DefaultRouter`**  
- **Расширяет `SimpleRouter`**, добавляя **автоматический `root-view`** (`/api/`).  
- Это удобный роутер, потому что он **показывает список всех зарегистрированных эндпоинтов**.  
...
Теперь при заходе на `/api/` можно увидеть список всех доступных маршрутов.  

📌 **Когда использовать?**  
✅ Если нужен **автоматический список API-эндпоинтов** (`root-view`).  

---

### **Вывод:**  
| Класс            | `BaseRouter` | `SimpleRouter` | `DefaultRouter` |
|-----------------|-------------|---------------|---------------|
| Используется напрямую? | ❌ Нет | ✅ Да | ✅ Да |
| Автоматическое добавление API root (`/api/`)? | ❌ Нет | ❌ Нет | ✅ Да |
| Можно регистрировать `ViewSet`? | ❌ Нет | ✅ Да | ✅ Да |

🔹 **Используйте `DefaultRouter`, если нужен root API**.  
🔹 **Используйте `SimpleRouter`, если root API не нужен**.
