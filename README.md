...
* токен авторизации
  ...
  + позволяет идентифицировать пользователя без необходимости хранения состояния на сервере
  + используется в API и системах с аутентификацией без сохранения состояния (stateless authentication)
  ...
  + Пользователь получает токен после входа в систему и передаёт его с каждым запросом
    - Сервер проверяет токен, чтобы убедиться, что пользователь авторизован
...
| **Характеристика**       | **CSRF-ключ**                  | **Сессионный ключ**            |
|---------------------------|--------------------------------|---------------------------------|
| **Назначение**           | Защита от CSRF-атак           | Идентификация пользовательской сессии |
| **Хранение в cookie**     | Да, читается клиентом         | Да, доступен только серверу    |
| **Имя cookie**            | `csrftoken`                  | `sessionid`                   |
| **Доступен JavaScript**   | Да                            | Нет (обычно `HttpOnly`)        |
| **Срок действия**         | Обычно длительный             | Настраивается, часто короткий  |

Here’s the same comparison table translated into English:
...
* 'django.contrib.messages'
  ...
  + подключается и `MessageMiddleware`
