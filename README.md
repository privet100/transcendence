...
* SSL/TLS 
  ...
  + nginx установливет **канал связи** SSL/TLS между сервером и браузером, чтобы шифровать трафик
...
* Пользователь открывает https://tr.naurzalinov.me/
  ...
  + Если через HTTP, передаёт в открытом виде 
  ...
  + если данные из Django (JSON, HTML, WebSocket), проходят через прокси на 8000 через **HTTP (внутренний трафик не шифруется)**
