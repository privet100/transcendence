...
* Папка backend/
  + Это основная часть бэкенда на Django (или другом фреймворке)
...
* Папка frontend/
  + certif/ и etc/  папки для сертификатов, дополнительных конфигураций, скриптов, вспомогательных файлов
...
4. Как всё вместе работает
Docker-compose поднимает несколько сервисов (контейнеров):

backend: Собирается из backend/Dockerfile, внутри запускается Django-приложение (Gunicorn или uvicorn, если это Channels).
frontend: Собирается из frontend/Dockerfile, внутри (если это dev-режим) может крутиться webpack dev server, или, если production, собранный фронтенд обслуживается Nginx.
(Возможно) db: Контейнер с базой данных (Postgres/MySQL).
(Возможно) nginx: Отдельный контейнер Nginx, который проксирует на backend и frontend. (Иногда он объединён с фронтом — зависит от архитектуры.)
.env подгружается** в docker-compose** или в Django, оттуда Django получает секреты (SECRET_KEY, ключи OAuth, данные для БД, и т.д.).

При разработке вы можете:

Менять код в backend/frontend.
Запускать docker-compose up --build (или update_docker.sh), чтобы пересобрать образы.
Django подхватывает изменения (если настроен hot-reload), фронтенд тоже.
В продакшене (production) при пуше в репозиторий (GitHub) может срабатывать webhook (backend/webhook/), который:

Принимает POST-запрос от GitHub, проверяет подпись.
Делает git pull и перезапуск docker-compose, чтобы обновить приложение «на лету».
Короткое резюме
backend/ — это Django-приложение (несколько подпроектов-«приложений» в Django, плюс «основной» проект в myproject). Тут логика чата, авторизации, API и т.д.
frontend/ — это весь клиентский код (CSS, JS, либо отдельный React/ Vue-сборщик, плюс Dockerfile для сборки).
docker-compose.yml и папки с Dockerfile-ами объединяют всё это в контейнеры, позволяя запускать проект единым «командным центром».
Nginx (через nginx.conf) выступает в роли прокси/сервера статики.
webhook/ даёт автоматический деплой при пушах в репозиторий (CI/CD).
.env и requirements.txt — настройки окружения и зависимости.
Такое разделение даёт удобную архитектуру для команды (бэкенд-программисты работают в backend/, фронтенд-программисты — в frontend/), а Docker упрощает развёртывание и воспроизводимость окружения.
...
* бэкенд или фронтенд,зависит от архитектуры вашего проекта и того, как именно вы собираетесь отдавать статику (CSS, JS, картинки). нередко делают так:
  ...
  + Monolith (только Django + шаблоны):
    ...
    - шаблоны на Django
    - CSS-файлы, картинки и т. д. логично хранить в Django-приложении в папке static/
