...
* 2) Установка защищённого соединения (TLS-рукопожатие)
  + браузер отправляет запрос для установки HTTPS-соединения и ранее полученный `session id`
  + nginx генерирует идентификатор сессии TLS **session id** (если это новое соединение, для возобновления сеанса в рамках одной TCP-сессии)
    - сохраняет  
    - отправляет браузеру   
  + браузер сохраняет session id в оперативной памяти
  + nginx отправляет браузеру .crt  
  + браузер сохраняет .crt  в кэше сессии
    - сверяет с CA, подтверждает подлинность сервера (1-я функция SSL – Аутентификация сервера)
  + браузер генерирует **pre-master secret** (предварительный ключ для симметричного шифрования)  
    - шифрует с использованием .crt  
    - отправляет серверу
    - не сохраняет
  + nginx расшифровывает pre-master secret с помощью`.key и .crt
    - не сохраняет
  + nginx и браузер независимо генерируют **сессионный ключ** (симметричный ключ, symmetric keys, derived key) на основе pre-master secret  
    - для шифрования последующего трафика
    - хранятся в памяти браузера, удаляются после завершения сессии
