...
* `settings.py` конфиг Django на высоком уровне
  ...
  + `AUTH_PASSWORD_VALIDATORS` валидаторы сложности паролейл (минимальная длина, ...)
  + logging.basicConfig
    ...
    + может конфликтовать с их настройками логгеров Django и Channels
...
* DPR, Data Processing and Rendering (Обработка и отображение данных) - данные (JSON, ...) обрабатываются на серверной стороне с помощью Django и передаются в представление для рендеринга на клиентской стороне
...
* суперпользователь Django Admin
  + доступ к админке
  + все права в системе
  + доступ к системе управления данными
  + редактирование, добавление, удаление записей во всех моделях, в базе через удобный интерфейс
  + можно для настройки данных или исправления ошибок в продакшене
...
* модуль сериализации данных для работы с JSON или API #see
  ...
  + позволяет создавать сложные структуры и проводить валидацию данных
  + django без DFR не предоставляет встроенного модуля сериализации 
...
Throttle Classes, ViewSets, Routers и Filters — это дополнительные, но важные компоненты DRF, которые работают в тандеме с модулями сериализации, аутентификации и прав доступа. Однако их нельзя напрямую назвать "модулями" наравне с тремя основными. Вот как они вписываются в архитектуру:
* вспомогательный компонент Throttles
  + контролируют частоту запросов пользователей
  + функциональный слой защиты от DoS-атак и чрезмерной нагрузки на сервер (**nginx недостаточно?**)
* вспомогательный компонент ViewSets
  + помогает автоматизировать и упрощать маршрутизацию RESTful API
  + объединяют логику CRUD-операций в одном классе
* вспомогательный компонент Routers
  + помогает автоматизировать и упрощать маршрутизацию RESTful API
  + автоматически связывают URL-маршруты с ViewSets
* вспомогательный компонент Filters  
...
* поддержка **сериализации**, аутентификации, прав доступа
...
* 'django.contrib.messages'
  ...
  + обмен данными
    - через ws соединения  
    - или через HTTP-запросы, добавляя сообщения во время перенаправлений или в шаблоны
...
* WebSockets
  ...
  + consumer отправляет **через WebSocket** обратно пользователю `{ "type": "chat.message", "message": "Hi there!" }` 
