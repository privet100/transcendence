...
* CSRF (Cross-Site Request Forgery) token
  ...
  + https://docs.djangoproject.com/en/5.1/howto/csrf/ CSRF с Django
* JWT (JSON Web Token)
  ...
  + лёгкий для передачи через URL, заголовки, ...
  ...
  + все данные для проверки токена уже содержатся в нём, нет необходимости хранить их на сервере
  + Использование
  ...
  + nginx хранит jwt безопасным способом, не в cookie, а в **локальном хранилище** (в переменной в js, обновляем как только он истечет)
...
* WSGI, Web Server Gateway Interface
  ...
  + помимо приложений и серверов, стандарт дает определение middleware-компонентов
    ...
    - могут: обработка сессий, аутентификация/авторизация, управление URL (маршрутизация запросов), балансировка нагрузки, пост-обработка выходных данных (проверка на валидность), изменение заголовков, ...
...
* перехожу на asgi only:
  ...
  + убедитесь, что nginx поддерживает WebSocket, добавьте заголовки в блок location:
    ...
    ```
* интегрировать и запустить Django с ASGI
  + ASGI не меняет Django-кода для HTTP, все стандартные Django-функции (middleware, views, models) продолжают работать в синхронном режиме
  ...
  + Тестирование
    - убедитесь, что стандартные HTTP-запросы работают через ASGI-сервер (загрузка HTML-страницы, отправка данных формы)
    - используйте функционал, связанный с WebSocket (чат, ...)
