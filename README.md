...
* JWT тоже могут использоваться для аутентификации пользователей
  ...
  + нужно вручную обрабатывать его в consumer'е
* **полагается на стандартную инициализацию Django для HTTP-запросов**
* для работы с wss:// сервер использует HTTPS с SSL-сертификатом
...
* не сервер
* указывает на использование сервера (Redis) для выполнения функций межпроцессного взаимодействия
* настройка Django Channels
  + как будут обрабатываться сообщения 
  + какой бэкэнд используется для обмена сообщениями между различными частями вашего приложения (например, между ws-соединениями и **фоновыми задачами**)
* позволяет разным инстансам или процессам приложения обмениваться сообщениями
* необходимо если приложение развернуто на нескольких серверах
* необходимо если требуется рассылка сообщений или уведомлений всем клиентам (или группам клиентов)
* зачем нужен
  + Хранение каналов: Обеспечивает регистрацию и управление подключениями (каналами) пользователей.
  + Групповая коммуникация: Позволяет создавать группы пользователей и отправлять сообщения всем участникам группы.
  + Межпроцессное взаимодействие Если приложение работает на нескольких инстансах (серверах), Channel Layers обеспечивает их синхронизацию.
* CHANNEL_LAYERS 'BACKEND': 'channels.layers.InMemoryChannelLayer', **in-memory ?**
* Django Channels использует Redis через установленный backend
...
* для распределения сообщений и управления группами пользователей, когда запросы обрабатываются несколькими серверами
...
* хранение состояния чата, управление уведомлениями
  + при отправке сообщения оно сначала отправлено в Channel Layer, затем пользователям через WebSocket
* механизм группировки пользователей (groups)
  + передавать сообщения из одного WebSocket-соединения в несколько других, которые принадлежат группе
* распределенные задачи (уведомления, обработка сообщений или других асинхронных операций): решать эти задачи в масштабируемом виде
...
* синхронизировать данные и события между серверами
  + если приложение работает на нескольких серверах (контейнерах), каждый из которых может обрабатывать запросы пользователей
...
* находит другие серверы благодаря использованию брокера сообщений (Redis), который позволяет передавать сообщения между различными инстансами приложения, даже если они работают на разных серверах
...
* использует Redis (или другой брокер сообщений) для обмена сообщениями между инстансами приложения
  + Каждый сервер или контейнер подключается к Redis
  + все сообщения передаются через этот брокер
* Масштабируемость
  + Если приложение работает на нескольких серверах или контейнерах, использование Redis как Channel Layer позволяет масштабировать приложение горизонтально. Вы можете добавлять новые
* предоставляют инфраструктуру для асинхронного обмена сообщениями, интегрированную с Django Channels. Это снимает необходимость вручную управлять:
...
* обмен сообщениями между различными инстансами приложения или между **различными процессами** (если их несколько)
  + **сообщениям между клиентами**
...
* daphne подключается к Redis-серверу
  ...
  + Предположим, несколько серверов/контейнеров обрабатывают ws-запросы
  + Когда один сервер получает сообщение от клиента, он может отправить его в канал через Channel Layer
  + Redis передает сообщение всем другим инстансам, которые подписаны на канал
...
### redis как backend для сhannel layers  
* хранение и маршрутизация сообщений между ws-клиентами, между различными инстансами вашего приложения (брокер)
  + каждый инстанс/контейнер подключается к Redis
  + redis = централизованный канал для передачи сообщений
  + Redis = посредник, через него один инстанс приложения отправляет сообщение в канал - другие инстансы, подписанные на канал, получат
...
* можно **заменить на другой бэкэнд** (PostgreSQL, RabbitMQ)
...
* настройте Redis
  ...
  + защищён (настройка пароля, ограничение доступа, ...)
