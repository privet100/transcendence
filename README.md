...
* приходит запрос к http://<ваш IP>/ на контейнер Nginx, порт 443
...
### - **`daphne`** — это ASGI-сервер для Django Channels.


### несколько подпроектов-приложений backend
* Django app
* отдельный модульный приложения внутри проекта
* отвечает за конкретную часть функционала, бизнес-логика (модели, вьюхи, сериализаторы, формы), тесты, админ-конфигурация и т. п.  
...
* myapp
  + отвечает за бизнес-логику пользовательских профилей, турниров и историй игр
  + модель `UserProfile` хранит информацию о пользователе, его дружеских связях (many-to-many на самого себя), аватаре, дате последней активности и т. д
  + модель `Tournament` описывает понятие «турнира», содержит список участников (many-to-many к `UserProfile`)
