...
#### 6) enable an HTTPS connection for all aspects
* wss instead of ws
* **http2 ?** 
...
* middleware ?
#### 9) csrf (cross-site request forgery) ключ, токен
* случайная строка
...
Нет, это **не CSRF-токен**, а **секретный ключ Django (`SECRET_KEY`)**.  

### Разница между `SECRET_KEY` и CSRF-токеном:  

| **Характеристика**     | **SECRET_KEY** | **CSRF-токен** |
|----------------------|--------------|-------------|
| Где хранится?      | В `settings.py` или `.env` | В cookie или передаётся в форме/заголовке |
| Для чего нужен?    | Шифрование, подпись JWT, сессий, паролей | Защита от CSRF-атак |
| Доступен клиенту? | ❌ Нет, скрыт на сервере | ✅ Да, передаётся в ответах сервера |
| Можно менять?      | ❌ Только если сбрасываете все сессии и подписи | ✅ Генерируется заново для каждой сессии |

### **Что делает `SECRET_KEY`?**  
...
### **CSRF-токен – это другое**  
CSRF-токен создаётся автоматически Django (`csrftoken` в cookie/форме) и проверяется при POST-запросах, но он **не имеет отношения к `SECRET_KEY`**.  

### **Вывод:**  
🔹 **`SECRET_KEY` – это главный секретный ключ Django, а CSRF-токен используется отдельно для защиты форм от подделки.**
