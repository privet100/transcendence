...
* myproject/
  ...
  + `settings.py` 
    ...
    - `AUTH_PASSWORD_VALIDATORS` валидаторы сложности паролей (минимальная длина, отсутствие совпадений с логином, ...)
...
* WSGI, Web Server Gateway Interface
  ...
  + Если вы используете только HTTP-запросы (API, стандартные страницы, ...), достаточно WSGI
...
* Если в проекте одновременно используются синхронные HTTP-запросы и асинхронные WebSocket, вам понадобится ASGI, так как ASGI поддерживает оба вида запросов
...
* ASGI-сервер может обрабатывать все виды запросов: поддерживает как асинхронные (WebSocket), так и синхронные (HTTP) запросы
  + WSGI всё ещё нужен в некоторых случаях, особенно если вы работаете с инструментами или средами, которые изначально поддерживают только синхронные HTTP-запросы
* Зачем нужен WSGI?
  ...
  + Для стандартных Django-приложений без асинхронных функций (например, без WebSocket) WSGI проще и стабильнее
  + Использование WSGI уменьшает сложность, если вам не нужны возможности ASGI
