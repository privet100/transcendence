...
* `TourPage.js: `createTour()`
  ...
  + в запросе данные о турнире: имя турнира, время начала, игроки
* `TourView`: обработчик `post`
  + получает запрос
  + создает новый турнир через сериализатор `TourSerializer`
  + вызывается метод `create_games()` для создания матчей (каждый с каждым)
* models.py: `invite_players()`
  + отправляет сообщения с приглашением игрокам через `ChatMessage`
  + после истечения времени (5 минут), если количество игроков больше 1, создаются игры для турнира через метод `create_games()`
* models.py: `create_games()`
  + для каждого игрока создается матч с каждым другим игроком турнира
