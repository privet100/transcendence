...
* AUTH_USER_MODEL = 'myapp.UserProfile`,  кастомная модель `myapp.UserProfile` вместо стандартного `User`
  ...
  + позволяет глобально изменить пользователя
  + таблица пользователей `myapp_userprofile` (не `auth_user`)
    **public** | auth_group                         | table | myuser
    public | auth_group_permissions             | table | myuser
    public | auth_permission                    | table | myuser
    public | myapp_userprofile_user_permissions | table | myuser
  + `username` `max_length=200` вместо `150` 
  + `email` Сделан обязательным и уникальным 
  + `friends`           Добавлено                   
  ...
  + | Функциональность          | Возможно в стандартной `User`? | **Как реализовать без расширения?** |
    |---------------------------|--------------------------------|---------------------------------|
    | `avatar`                  | Нет                            | Создать модель `UserProfile`, связанную с `User` через `OneToOneField` |
    | `friends`                 | Нет                            | Создать модель **Friendship**, связанную `ManyToManyField(User)` |
    | Уникальный `mail          | Нет (по умолчанию)             | Переопределить `UserManager`, сделать `email` `unique=True` |
    | URL для фото picture_url  | Нет                            | Добавить поле в `UserProfile` |
    | username до 200 символов  | Нет (`max_length=150`)         | Переопределить модель, изменить `max_length` |
...
* ошибка "role 'postgres' does not exist"
  + user_permissions = models.ManyToManyField влияет на систему аутентификации Django
    - из-за некорректной ManyToMany-связи с `Permission` Django неправильно создал миграции и не создал пользователей
    - django создает нового пользователя при миграции
    - django использует модель `User` для аутентификации и управления пользователями
    - вы наследуете `UserProfile` от `AbstractUser` -> django создает таблицу пользователей и автоматически обрабатывает разрешения (`user_permissions`)
    - по умолчанию, `AbstractUser` уже содержит связь `user_permissions`, но без `related_name`
    - если вы не указываете `related_name`, Django может запутаться при обработке ManyToMany-связи
    - без `related_name` миграции базы данных проходили неправильно, `superuser` не создавался -> ошибка при попытке подключения backend-а к PostgreSQL
  + добавление `related_name` -> django понимает, как связана модель `UserProfile` с `Permission` ->
    - это предотвращает конфликты при создании миграций
    - это обеспечивает корректное создание пользователей в базе данных
    - это позволяет создать суперпользователя и избежать ошибки "role 'postgres' does not exist"
  + создайте суперпользователя после применения миграций: `docker-compose exec backend python manage.py createsuperuser`
    - если после этого ошибка исчезнет, значит, проблема действительно была в неправильном создании модели пользователя
  + bakyt: Django Admin Compatibility: The Django admin uses groups and user_permissions for user permission management. Removing these fields will disable group/permission-related functionality in the admin. If you still need the admin, avoid removing these fields or use a custom admin panel.
  + bakyt: Superuser Creation: The createsuperuser command may fail if your user model lacks these fields. To fix this, override the createsuperuser command or ensure your model has no permission-related dependencies.
...
* ТОКЕН АВТОРИЗАЦИИ JWT (основной вариант это JWT)  
  + jwt = формат токена
  + для Веб-приложения, API, ws, stateless-приложений, систем с аутентификацией без сохранения состояния (stateless authentication), SPA, мобильных приложений, микросервисов
  + сервер проверяет данные и создаёт JWT, содержащий информацию о пользователе (id, имя, ...)
  + клиент получает токен и отправляет его в заголовке `Authorization`, через заголовки ws-сообщений или параметры URL
  ...
  + может храниться в HTTP заголовках, cookie, localStorage, sessionStorage
    - хранится в браузере, поэтому безопасно **почему у нас не так?**
  ...
  + не нужен CSRF-токен
  + не нужны сессии, если вся информация хранится в токенах
  + если используется исключительно токен-авторизация (JWT), можно **отключить SessionMiddleware и CsrfViewMiddleware**
...
* 42 oauth = разновидность токен-ориентированной аутентификации 
  ...
  + если хранится на стороне клиента в Local Storage
    ...
    - злоумышленники могут украсть токен через js-код (XSS-атака)
  ...
  + если хранится на стороне сервера в базе данных (**Access Token, Refresh Token**)
    - безопасность: токен недоступен напрямую для клиента
    - удобное управление сроком действия и обновлением токенов
    - доступ к базе данных может быть медленнее
  ...
  + если требуется высокая безопасность: храните Access и Refresh Tokens на сервере (в базе данных или Redis), используйте HTTP-only cookies для передачи сессионных данных
  + если важно удобство разработки: Local Storage или Session Storage
  + Frontend отправляет запросы к Backend
    - backend использует токены для взаимодействия с внешними сервисами без необходимости передачи токенов на Frontend
