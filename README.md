...
| **токен/ключ**   | **Назначение**                                                  | **Где хранится**                      | **Особенности**                                                                                     | **Пример использования**                                                                                                                                       |
|------------------|-----------------------------------------------------------------|---------------------------------------|-----------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CSRF-токен         | Защита от подделки запросов (CSRF-атак)                      | cookie csrftoken                       | - Проверяется сервером при POST/PUT запросах.<br>- Хранится в cookie и передаётся в скрытом поле формы или заголовке. | - Защита формы входа.<br>- Обеспечение легитимности запросов от пользователя.                                                                                |
| Токен авторизации  | Подтверждение авторизации пользователя                       | HTTP-заг cookie locStorage sessStorage | - Используется для REST API и WebSocket.<br>- Может быть JWT или иным форматом.                                           | - Авторизация REST API (например, `Authorization: Bearer <token>`).<br>- Авторизация WebSocket-соединения.                                                   |
| Сессионный ключ    | Управление пользовательской сессией                          | cookie `sessionid`                     | - Долговременная связь пользователя с сервером.<br>- Подходит для веб-интерфейсов.<br>- Сохраняется на сервере.         | - Отслеживание состояния авторизации в веб-приложении.<br>- Сохранение данных пользователя (корзина, настройки).                                              |
| WebSocket-токен    | Авторизация пользователя при установке ws-соединения         | URL параметр или заголовок ws-запроса  | - Передаётся при установке соединения.<br>- Необходимо для проверки прав доступа.                                         | - Авторизация чата или потоковой системы (например, `wss://example.com/ws/chat/?token=<your-token>`).                                                         |
| Email-токен        | Подтверждение email-адреса, восстановление пароля, ...       | URL отправляемое пользователю          | - Одноразовый токен.<br>- Генерируется для конкретного действия.                                                       | - Подтверждение email через ссылку: `https://example.com/verify-email/?token=<your-token>`.                                                                   |
| API-ключи          | Идентификация и авторизация сторонних приложений, интеграций | HTTP заголовок запроса                 | - Передаётся с каждым запросом.<br>- Часто имеет ограниченный доступ к API.                                              | - Интеграция с внешним сервисом или предоставление публичного API (например, `Authorization: Api-Key <your-key>`).                                            |
| Шифровальные ключи | шифрование сообщений                                         | На сервере в хранилище                 | - Не передаётся клиенту.<br>- Используется для шифрования/дешифрования.                                                  | - Шифрование сообщений чата на стороне сервера.<br>- Защита конфиденциальных данных.                                                                          |
| OAuth-токены       | Аутентификация и авторизация через Google                    | В cookie или localStorage              | - Используется для работы с OAuth 2.0.<br>- Подтверждает доступ пользователя к сервису.                                  | - Авторизация через Google OAuth API.<br>- Доступ к данным пользователя через API (например, получение email с разрешения пользователя).                     |
...
* после daphne
  ...
  +  Consumer (Channels Consumer)
    - обработчик сообщений WebSocket в Django Channels (аналогичные представлениям в традиционном Django)
  + Backend (Redis / Channels Layer)
    - могут быть отправлены через Channel Layer (Redis)
     - для обмена сообщениями между различными экземплярами вашего приложения, например, для отправки сообщений между WebSocket-соединениями.
    - Когда данные отправляются в **channel layer**, они могут быть опубликованы в определенный канал и доставлены соответствующему потребителю, который подписан на этот канал
...
* Когда в проекте используется **Django Channels** для обработки WebSocket-запросов (например, для чата), **Channel Layer** нужен для организации обмена сообщениями между различными инстансами приложения или даже между различными процессами (если их несколько). Хотя **Django Channels** позволяет обрабатывать WebSocket-соединения, **Channel Layer** используется для управления **сообщениями между клиентами**, а также для связи между компонентами вашего приложения.

Вот основные причины, зачем вам может быть нужен **Channel Layer** после обработки WebSocket-запросов:

### 1. **Обмен сообщениями между инстансами сервера**
Если ваше приложение масштабируется и работает на нескольких серверах (или контейнерах), каждый из которых может обрабатывать запросы пользователей, то **Channel Layer** (с использованием **Redis** или другого бэкенда) позволяет синхронизировать данные и события между этими серверами. Это важно для обработки сообщений чатов и уведомлений, если они должны быть доступны всем пользователям, подключенным к разным серверам.

- Например, если два пользователя находятся на разных серверах, и один отправляет сообщение через WebSocket, то без **Channel Layer** это сообщение не будет доставлено на другой сервер.
- **Channel Layer** решает эту задачу, передавая сообщения между серверами через **Redis** (или другие бэкенды), чтобы гарантировать, что все пользователи, подключенные к вашему приложению, получат актуальную информацию.

### 2. **Рассылка сообщений всем клиентам (или группе клиентов)**
Когда пользователь отправляет сообщение в чат, это сообщение может быть предназначено не только ему, но и всем участникам чата. В этом случае, **Channel Layer** может использоваться для отправки сообщения всем подключенным пользователям, кто является частью той же группы (например, чата).

- **Channels** обеспечивают удобный механизм группировки пользователей (через **groups**), так что вы можете легко передавать сообщения из одного WebSocket-соединения в несколько других, которые принадлежат одной группе.
  
  Пример использования группы в **Channel Layer**:
  ```python
  # В consumer
  self.channel_layer.group_send(
      "chat_group",  # Название группы
      {
          "type": "chat.message",
          "message": "Hello, World!",
      }
  ...
  ```
  Это сообщение будет отправлено всем пользователям, подключенным к группе `chat_group`.

### 3. **Сохранение состояния чата и уведомлений**
**Channel Layer** также может использоваться для хранения состояния чата и управления уведомлениями. Например, при отправке сообщения оно может быть сначала отправлено в **Channel Layer**, а затем доставлено соответствующим пользователям через WebSocket.
