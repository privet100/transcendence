...
* какой у нас тип - посмотреть auth, authentication, REST_FRAMEWORK, oauth, session, cookies, bearer, authorization, csrf, middleware, authentication classes   
  ...
  - Если сервер выставляет куку (`Set-Cookie: sessionid=...`) => сессионная аутентификация (Django Sessions, ...)
...
| Как работает                                       | Где хранится аутентификационная информация?  
| сервер выдаёт токен, клиент передаёт в заголовках  | `localStorage`/`sessionStorage`, куки
* можно украсть токен
...
| Как работает                                       | Где хранится аутентификационная информация?
| пользователь логинится через 42, получает токен    | Сторонний OAuth-сервер выдаёт токен


#### API-КЛЮЧИ
+ OAuth: пользователи логинятся в твой проект, используя учетные данные 42
  - зарегистрировать приложение в 42 и получить данные для подключения: (API-ключ) или (клиентский ID и секрет)
* удобно для сервисов, API
* можно украсть ключ
| Как работает                                       | Где хранится аутентификационная информация?  
| выдаётся уник. ключ (как токен), передаётся в загол| клиент хранит ключ, сервер проверяет его в бд  
...
#### autorisation = что разрешено делать, к каким ресурсам имеете доступ 
