...
* myapp/  
  + отдельное «модульное» приложение, отвечающее за конкретную часть функционала (например, чат, личный кабинет, API и т.д.)
  + одно из приложений внутри проекта (Django app)  
...
* Nginx (через nginx.conf) в роли прокси/сервера статики
* Nginx — серверный компонент, кажется логичным хранить его конфигурацию в папке, связанной с бэкендом. Но тут `nginx.conf` лежит в папке `frontend` по следующим причинам:
  + Docker-контейнер для фронтенда  
   - В проекте Transcendence часто делают отдельный контейнер с Nginx, который обслуживает именно «фронтовую часть» приложения: статические файлы (HTML, CSS, JS) и проксирует запросы к бэкенду
  ...
  + Обслуживание статических файлов  
   ...
   - В `nginx.conf` вы можете видеть строки типа: `location /static/ { alias /usr/share/nginx/html/static/; }`, это «логика фронта», поэтому конфигурация хранится там же
...
### SSL
* сертификаты в /etc/nginx/ssl и /certif
