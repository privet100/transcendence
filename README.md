...
* для DefaultRouter и маршрутизации
* стандартный модуль сериализации
* стандартный модуль сериализации аутентификации
...
* API = группа маршрутов (эндпоинтов)
* интерфейс, набор правил
* обрабатывает стандартные HTTP-запросы (GET, POST, PUT, DELETE)
...
* с поддержкой **сериализации**, аутентификации, прав доступа
...
* Endpoint = URL-маршрут = конкретный маршрут, связанный с API
  ...
  + Для эндпоинтов, которые возвращают HTML (главная страница, панель администратора), откройте в браузере для проверки
...
* 'django.contrib.messages'
  ...
  + обмен данными через ws соединения  
...
* WebSockets
  ...
  + `{ "type": "chat.message", "message": "Hello, World!" }`
    ...
    - nginx
    - daphne
    - в AuthenticationMiddlewareStack
    - в SessionMiddleware (если используется) работа с сессией
    - механизм Channels
    - consummer
    - Backend Channel Layer обмен данными через канал
