...
* Пользователь открывает в браузере https://tr.naurzalinov.me/
  + приходит запрос к http://<ваш IP>/
  + запрос попадает на фронтенд-контейнер Nginx, порт 443
    - если запрос к статическому файлу (CSS/JS/картинк/HTML), nginx отдает его из /usr/share/nginx/html/static/
    - если /, /api/…, nginx проксирует запрос на backend:8000, Django обрабатывает API
...
* Пользователь, запрашивая `https://tr.naurzalinov.me`, устанавливает защищённое соединение с Nginx, а Nginx перенаправляет запросы на бэкенд по внутреннему протоколу `http://backend:8000`
...
* Запускает команду runserver 0.0.0.0:8000 => запускает Django-приложение
* Открыт на порту 8000 внутри сети Docker
* Django-сервер слушает внутри контейнера на 8000 порту
* Бэкенд-фреймворк
* обрабатывает запросы от фронтенда и формирует ответы — будь то рендеринг шаблонов (Server-Side Rendering) или отдача JSON-данных (REST API)
* Принимает запросы  от Nginx
* отдаёт JSON/HTML или websocket (Channels) — в зависимости от реализации
* бизнес-логика (чат, авторизация, API, управление базой данных, игровая логика, ...)
* бизнес-логика, роутинг Django, модели, чаты, аутентификация
...
* несколько подпроектов-«приложений»
  ...
  + myapp общее приложение или ваша основная бизнес-логика
* myproject/
...
* myapp/  
  + Django app, отдельное «модульное» приложение внутри проекта, отвечающее за конкретную часть функционала (например, чат, личный кабинет, API и т.д.)
  + бизнес-логика (модели, вьюхи, сериализаторы, формы), также тесты, админ-конфигурация и т. п.  
  + `models.py`,  
  + `views.py`,  
...
* staticfiles/ Django или система сборки складывает статические файлы после команды collectstatic
...
* серверная логика, API, связь с БД и Redis
...
* слушает на портах 80/443, отвечает на порты 80/443
...
* проксирует API-запросы (например, http://localhost/api/...) на бэкенд
* в роли прокси/сервера статики
* `nginx.conf` лежит в папке `frontend` по следующим причинам:
  + Docker-контейнер для фронтенда  
   - отдельный контейнер с Nginx обслуживает именно «фронтовую часть» приложения: статические файлы (HTML, CSS, JS) и проксирует запросы к бэкенду
   - В `docker-compose.yml` этот сервис, как правило, назван `frontend` и использует Dockerfile из папки `frontend/`
   - Следовательно, логично хранить `nginx.conf` рядом с Dockerfile, чтобы во время сборки он копировался внутрь контейнера
  + Обслуживание статических файлов  
   ...
   - В `nginx.conf` вы можете видеть строки типа: `location /static/ { alias /usr/share/nginx/html/static/; }`, это логика фронта, поэтому конфигурация там же
  + Проксирование бэкенда  
   ...
   - Несмотря на то, что Nginx выполняет «серверную» задачу проксирования, он всё равно обслуживает именно фронтенд-контейнер. `nginx.conf` часть «внешнего (front)» эшелона приложения
  + Условное деление директорий  
   - В современном «multi-container» окружении (Docker, k8s) папка `frontend/` может содержать всё, что касается сервировки фронта: код (если у вас React/Vue/Angular) или статику, а также конфиги веб-сервера (Nginx)
   - Папка `backend/` сосредоточена на коде Django (или другого фреймворка), его настройках, requirements, миграциях и т.д
   - Разделение на «frontend» и «backend» в данном случае — это просто логическая структура, а не строгая привязка к тому, что «фронтенд = работает в браузере, а конфиги = сервер»
  + Удобство разработки  
   - В Dockerfile для frontend вы обычно увидите:
     ```
     FROM nginx:latest
     COPY nginx.conf /etc/nginx/conf.d/default.conf
     COPY ./static /usr/share/nginx/html/static
     ```
   - То есть вся «сборка» фронта происходит из одной папки
  + `nginx.conf` в папке `frontend/`, потому что в Docker-архитектуре этот сервис называется `frontend` и отвечает за раздачу статики и проксирование запросов к бэкенду 
  + Такое расположение логически объединяет весь код и конфиги, необходимые для сборки и запуска фронтенд-контейнера (Nginx)
* Nginx обеспечивает HTTPS (SSL/TLS) и выступает как обратный прокси (reverse proxy) для вашего бэкенда
