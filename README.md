...
* myproject/
  ...
  + `asgi.py`, `wsgi.py` **точки входа** Django для запуска приложения
  ...
  + `settings.py` 
    ...
    - `WSGI_APPLICATION` точка входа для WSGI-серверов, если запускаете проект в синхронном режиме (или для **совместимости**)
...
* WSGI, Web Server Gateway Interface
  ...
  + WSGI = простой и универсальный интерфейс между большинством веб-серверов и веб-приложениями или фреймворками
  + интерфейс, используемый Django-приложениями
  + WSGI-приложение должно удовлетворять следующим требованиям:
    - callable объектом (обычно это функция или метод)
  ...
  + помимо приложений и серверов, стандарт дает определение middleware-компонентов
    ...
    - Middleware — обертка над объектом приложения, реализовать её можно через замыкание
    - Middleware могут брать на себя следующие функции: обработка сессий, аутентификация/авторизация, управление URL (маршрутизация запросов), балансировка нагрузки, пост-обработка выходных данных (например, проверка на валидность)
изменение заголовков

  + Поддерживает только **синхронные** запросы HTTP
  + при запросе WSGI-сервер (Gunicorn, uWSGI, ...) передаёт данные о запросе Python-приложению
