...
### статические файлы PRODUCTION:NGINX
...
* `STATIC_URL = '/staticfiles/'` url для nginx, не название папки
...
* `python manage.py collectstatic`
  + собирает статические файлы (включая админку и DRF) в директорию STATIC_ROOT = app/staticfiles/
  + **не трекается гитом**
...
+ `location /staticfiles/ { alias /app/staticfiles/; }` # папка внутри контейнера Nginx
  + Nginx обслуживает статические файлы по пути, определенному в STATIC_URL в Django
  + Daphne не отдает статические файлы, передает запросы к Django-приложению, Django использует STATIC_URL для указания, где эти файлы

    - две отдельные директивы `location`
    - 1) location /static/ { alias /usr/share/nginx/html/static/;} }: собственные стат файлы в `/static` внутри контейнера Nginx
    - Nginx обслуживает файлы, запросы к которым начинаются с `/static/`.
    - Например, запрос на `/static/style.css` будет обслуживаться из `/usr/share/nginx/html/static/style.css`.
    - location /staticfiles/ { alias /app/staticfiles/; }  # стат файлы созданные Django собираются в `/app/staticfiles` в контейнере backend, Nginx проксирует запросы к Django


2. **Статические файлы, созданные Django**:
   - Папка `/app/staticfiles/` в контейнере **backend** (где **Django** собирает свои статические файлы после выполнения `collectstatic`).
   - **Nginx** обслуживает файлы, запросы к которым начинаются с `/staticfiles/`.
   - Например, запрос на `/staticfiles/admin/css/style.css` будет обслуживаться из `/app/staticfiles/admin/css/style.css`.

### Важно:

- Вам нужно будет создать общий том, чтобы **Nginx** и **backend** контейнеры могли обмениваться статическими файлами. Для этого можно использовать Docker volume.
...
services:
  backend:
    volumes:
      - staticfiles:/app/staticfiles  # Для Django статика

  frontend:
    volumes:
      - staticfiles:/usr/share/nginx/html/static  # Для Nginx статика

volumes:
  staticfiles:
```

Теперь у вас будет настроено два разных пути для обслуживания разных типов статических файлов, но оба пути могут начинаться с `/static/`. **Nginx** будет обслуживать их, правильно направляя запросы к нужным папкам на основе их URL.
...
* проверка 
  ...
  + `ls -la /app/staticfiles/` внутри контейнера - собранные файлы попали в `STATIC_ROOT` `/usr/share/nginx/html/staticfiles/`
  + `python manage.py collectstatic` внутри контейнера  
  + `docker exec -it frontend nginx -t` корректность конфигурации Nginx
