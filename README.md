...
* добавляет поддержку протокола WebSocket
  + обработка асинхронных протоколов, управление асинхронными соединениями
...
* WebSockets
  ...
  + `{ "type": "chat.message", "message": "Hello, World!" }`
    ...
    - consumer отправляет его через WebSocket обратно пользователю
    - `{ "type": "chat.message", "message": "Hi there!" }` отправляется обратно в Consumer **через WebSocket**
...
* позволяет различным частям вашего приложения общаться друг с другом асинхронно
* механизм для передачи сообщений между consumers и группами
* Позволяют отправлять сообщения группе подключений, например, всем участникам чата.
* распределят нагрузку между несколькими процессами или серверами
...
* отслеживает, какие клиенты подключены к каким группам (например, в чате)
* очереди сообщений
...
* без Channel Layer
  ...
  + Ручная маршрутизация сообщений: разрабатывать механизм для связи между клиентами (например, вручную управлять Redis Pub/Sub)
...
* убедитесь, что Redis работает: `redis-cli ping`, ответ: `PONG`
...
* интегрируется с Django, библиотеки абстрагируют низкоуровневую работу с Redis, 
