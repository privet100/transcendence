...
* `nginx.conf` в папке `frontend` по следующим причинам:
  ...
  + Удобство разработки  
   - В Dockerfile для frontend сборка фронта происходит из одной папки: COPY nginx.conf /etc/nginx/conf.d/default.conf, COPY ./static /usr/share/nginx/html/static
...
### приложения backend
* Django app - отдельное модульные приложения внутри проекта
