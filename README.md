...
* обработка API-запросов, HTTP-запросов через Django REST Framework (DRF) для выполнения бизнес-логики
...
* обращается к Redis для кэша, сессий, Pub/Sub
...
* `asgi.py`
  ...
  + URLRouter, ProtocolTypeRouter, маршрутизатор/обработчик ASGI-приложения: протокол (HTTP, WebSocket) для обработки запроса
    ...
    - "websocket": AllowedHostsOriginValidator(AuthMiddlewareStack(URLRouter(websocket_urlpatterns))) обрабатывает, `websocket_urlpatterns` из `chat/routing.py`: перенаправлять на `ChatConsumer`
* после daphne
...
- **Middleware:** В стандартном Django и DRF используются обычные HTTP middleware, например:
  - **SessionMiddleware** – управляет сессиями.
  - **AuthenticationMiddleware** – проверяет аутентификацию.
  - **CSRFMiddleware** – защищает от CSRF-атак.
- **ViewSet/APIView:** Обработчики HTTP-запросов, которые определяют, как данные обрабатываются для каждого типа запроса.
  - Пример в DRF:
    ```python
    ...
    ```
- **Использование:** DRF управляет запросами по HTTP и позволяет создавать API с поддержкой сериализации, аутентификации и прав доступа.

---

### Сравнение функций:

- **Аутентификация:** В обоих случаях аутентификация выполняется через middleware.
  - В **Django Channels** аутентификация осуществляется через `AuthenticationMiddleware`, который связывает подключение с пользователем.
  - В **Django REST Framework** аутентификация реализуется через стандартный механизм HTTP-запросов, например, с использованием JWT или сессий.

- **Сессии:** 
  - В **Django Channels** сессии также обрабатываются через `SessionMiddleware`, и сессионные данные передаются через WebSocket-соединения.
  - В **Django REST Framework** сессии управляются с помощью cookie-сессионных данных или токенов для аутентификации API.

- **Обработка запросов:**
  - **WebSocket (Django Channels):** Используется для асинхронной связи и поддерживает постоянные соединения, что позволяет получать и отправлять данные в реальном времени.
  - **HTTP (Django REST Framework):** Стандартный запрос-ответ модель, где каждый запрос обрабатывается отдельно, и клиент получает ответ сразу.

### Заключение:
- **Django Channels** — это решение для работы с асинхронными запросами и WebSocket-соединениями, подходящее для приложений, требующих реального времени (например, чат-приложения, уведомления).
- **Django REST Framework** — предназначен для создания API с использованием HTTP-запросов, который удобен для RESTful API и взаимодействия между клиентом и сервером в классическом веб-приложении.


Обе технологии могут использовать аналогичные принципы аутентификации и сессий, но с разными подходами, подходящими для их типов запросов (HTTP или WebSocket).

### 7. **Ответ в WebSocket (back to consumer)**
...
### Дополнительные возможные слои:

- **MessageMiddleware** (если используется) может быть настроен для обработки сообщений и добавления дополнительной информации или фильтрации на уровне WebSocket.
- В случае использования специфичных для Channels middleware, например, для ограничения скорости, управления правами доступа или обработки ошибок, они могут быть добавлены между слоями.

### Путь запроса WebSocket в Transcendence:

1. **Daphne** — принимает WebSocket-запрос.
...
### Важно:
- В случае использования **session storage** или **JWT**, эти данные могут быть доступны в middleware или в consumer.
- **SessionMiddleware** и **AuthenticationMiddleware** могут работать параллельно в случае использования сессий и аутентификации.

Таким образом, путь запроса WebSocket включает несколько слоев, обеспечивающих аутентификацию, авторизацию, обработку данных и их передачу через канал.
...
|---------------------------|----------------------------------|--------------------------------|---------------------------------|
| **Назначение**           | Аутентификация                  | Защита от CSRF-атак           | Идентификация сессии           |
| **Где хранится?**         | HTTP-заголовки, cookie, JS-хранилища | Cookie (обычно)               | Cookie (обычно)                |
| **Stateful/Stateless**    | Stateless (чаще)               | Stateful (требует серверную проверку) | Stateful                      |
| **Пример использования** | REST API, GraphQL, WebSockets   | Формы, AJAX-запросы           | Веб-приложения с авторизацией  |

### Сравнительная таблица токенов, ключей и их использования в веб-приложениях (на примере проекта Transcendence)

| токен/ключ    | Назначение                                       | Где хранится                         | Особенности | пример |
|---------------|--------------------------------------------------|--------------------------------------|-------------|--------|
| CSRF          | от подделки запросов (CSRF-атак)                 | cookie csrftoken                     | проверяется сервером при POST/PUT запросах, передаётся в скрытом поле формы или заголовке            | - Защита формы входа.<br>- Обеспечение легитимности запросов от пользователя.                                                                                |
...
| API-ключи     | идентификация и авторизация сторонних приложений | HTTP заголовок запроса               | передаётся с каждым запросом; ограниченный доступ к API                                          | интеграция с внеш сервисом, предос-ние публич API `Authorization: Api-Key <>`|
...
* хранятся внутри браузера
* Cookie
  + текстовые данные
  + хранятся в браузере, в разделе, отведённом для cookie
  + доступеы через сервер 
  + доступны через JavaScript (если нет флага `HttpOnly`)
  + автоматически отправляются серверу при каждом HTTP-запросе на соответствующий домен.
  + объём до 4 KB на cookie
  + срок действия может быть ограничен (до истечения времени или закрытия браузера).
  + использование:
