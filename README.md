...
* могут быть как стандартными REST (GET, POST, PUT, DELETE)
* могут быть асинхронными WebSocket (через Django Channels)
* описаны в `urls.py`
* определяются в View-классах или функциях, см. классы, которые наследуют от `APIView`, `GenericViewSet`, `ViewSet`
* `routing.py`- маршруты WebSocket:
  + Каждый WebSocket-путь можно считать отдельным API
* если у вас настроена автоматическая документация (Swagger, Redoc), вы можете увидеть все API через браузер     http://localhost:8000/swagger/, http://localhost:8000/redoc/
* Если есть приложения `api_42`, `chat`, `auth_app`, то для каждого из них может быть 5–10 эндпоинтов
* WebSocket тоже считается частью API
* Примерный список
  + API для аутентификации/авторизации (École 42, Google, и т. д.).
  + API для работы с чатом (отправка, получение сообщений).
  + API для обработки пользовательских данных (профиль, друзья, статистика).
  + API для игры (управление матчами, таблицы лидеров).
* API (Application Programming Interface) - интерфейс или набор правил, который позволяет приложениям взаимодействовать друг с другом
* API — группа маршрутов (эндпоинтов), которые клиент может использовать для взаимодействия с сервером
* один API состоит из нескольких эндпоинтов, которые реализуют функциональность API
* Например, ОДНО API для управления пользователями может включать маршруты
  - `GET /users/` — получить список пользователей
  - `POST /users/` — создать нового пользователя
  - `GET /users/<id>/` — получить информацию о конкретном пользователе
  - `DELETE /users/<id>/` — удалить пользователя
  - API чата = группа всех маршрутов, связанных с чатом
* Endpoint — это конкретный маршрут, связанный с API
  + Каждый endpoint выполняет определённое действие
  + Endpoints чата:
