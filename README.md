...
1. ASGI-сервер, например, **Uvicorn** или **Daphne**, обрабатывает все соединения.
2. Синхронные HTTP-запросы передаются стандартным Django-приложением.
3. WebSocket-запросы обрабатываются через Django Channels.

---

### Итог
- **Для стандартных Django-приложений** без WebSocket нужен только WSGI.
- **Для приложений с WebSocket или другими асинхронными задачами** (например, в реальном времени) вам нужен ASGI.
- **Transcendence**, судя по использованию Redis и настроек Django Channels, вероятно, использует ASGI для поддержки WebSocket. Поэтому ASGI предпочтительнее.
