...
* проверка личности, процесс логина, система требует пароль, одноразовый код OTP из SMS, отпечаток пальца, лицо, вход через соцсети, ...
...
* ТОКЕН АВТОРИЗАЦИИ JWT (основной вариант это JWT)  
  ...
  + для SPA, мобильных приложений, микросервисов
  + информация о пользователе (ID, ...)
  + самодостаточен, все данные для проверки токена содержатся в нём, без хранения состояния на сервере
    - не требует состояния на сервере (кроме валидации и возможной чёрной/белой списков)
  + подписан сервером, использует подпись, чтобы предотвратить изменения данных
  + может храниться в HTTP заголовках, в cookie, в localStorage, sessionStorage
    - у нас в локальном хранилище?
    - лучше, безопаснее, может храниться в браузере, поэтому безопасно **почему у нас не так?**
    - самый простой и безопасный — **cookie**, все микросервисы имеют одно и то же имя хоста => файл cookie отправляется браузером при каждом запросе
  + клиент после входа получает токен и отправляет его в заголовке `Authorization`, сервер убеждается, что пользователь авторизован
    - может быть передан через заголовки ws-сообщений или параметры URL
  + с jwt токеном не нужен CSRF-токен
...
* 42 oauth = разновидность токен-ориентированной аутентификации 
  ...
  + регулярно **обновляйик токены через OAuth2 Refresh Token**
