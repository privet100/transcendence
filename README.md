...
* docker-compose up --build (или update_docker.sh), чтобы пересобрать образы -> Django подхватывает изменения (если настроен hot-reload), фронтенд тоже

### ..
* контейнер backend (Django-приложение)
...
* контейнер redis (для кэша / сообщений)
