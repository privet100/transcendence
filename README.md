...
* `ssl_protocols TLSv1.2 TLSv1.3;` версии 
...
* `etc/nginx/ssl/` сертификаты для основного домена/сайта/приложения, где крутится Transcendence
  + `.csr` запрос на подпись сертификата (Certificate Signing Request): генерируете `.csr` с приватным ключом локально, отправляете `.csr` в удостоверяющий центр (Let’s Encrypt, ...), получаете `.crt`  
...
* внутри контейнера Nginx слушает 80 и 443
* а снаружи к frontend привязаны "80:80" и "443:443", это позволяет доступ извне по HTTP(80) и HTTPS(443)
...
* Пользователь открывает https://tr.naurzalinov.me/
  ...
  + если /, nginx проксирует запрос на backend:8000
    ...
    - осуществляет proxy_pass на бэкенд, т.е становится точкой входа для запросов (HTTPS/HTTP)
    - или перенаправляет /api запросы на backend:8000
...
* снаружи можно вызвать на localhost:8000
...
* обращается к Redis для кэша, сессий, Pub/Sub (чат, обновления статусов игроков в реальном времени)
  + подключается к Redis по адресу redis:6379
...
* myproject/
  ...
  + `settings.py` 
    ...
    - `STATIC_ROOT = staticfiles` куда Django или система сборки складывает статические файлы при выполнении `collectstatic` (**не трекается гитом**)
    - `SECRET_KEY` для криптографических подписей (в продакшене не выкладывают в открытый доступ) 
    - `DEBUG = True` режим отладки ( детальные ошибки)  
    - `ALLOWED_HOSTS` список доменов/IP, с которых разрешён доступ к этому Django-приложению (когда `DEBUG=False`)
    - `ASGI_APPLICATION` см. отдельный раздел
...
### Как запустить Daphne вместо `runserver`?

#### Новый вариант (использующий Daphne):
```dockerfile
CMD ["daphne", "-b", "0.0.0.0", "-p", "8000", "myproject.asgi:application"]
```

- **`daphne`** — сервер ASGI.
- **`myproject.asgi:application`** — путь к вашему приложению ASGI, определённому в файле `asgi.py`.

---

### Если у вас нет Daphne в проекте:
Добавьте его в зависимости (`requirements.txt`):
```plaintext
daphne
```
...
Команда `python manage.py runserver` запускает встроенный сервер разработки Django, но он **не подходит для работы с ASGI** и Daphne. Чтобы использовать Daphne в продакшене, замените `runserver` на команду `daphne` в вашем Dockerfile.
...
* WSGI, Web Server Gateway Interface
  ...
  + простой и универсальный интерфейс между веб-сервером и веб-приложениями или фреймворками, используемый Django-приложениями
  + WSGI-приложение должно удовлетворять требованиям:
    - callable объект (обычно это функция или метод)
    - принимать два параметра: словарь переменных окружения и обработчик запроса (start_response)
  ...
  + помимо приложений и серверов, стандарт дает определение middleware-компонентов
    ...
    - Middleware = обертка над объектом приложения
    - Middleware могут брать на себя функции: обработка сессий, аутентификация/авторизация, управление URL (маршрутизация запросов), балансировка нагрузки, пост-обработка выходных данных (например, проверка на валидность), изменение заголовков, ...
  + при запросе WSGI-сервер передаёт данные о запросе Python-приложению
  ...
  + всё ещё нужен в некоторых случаях
    - если вы работаете с инструментами или средами, которые изначально поддерживают только синхронные HTTP-запросы
    - многие устоявшиеся инструменты и сервисы (Gunicorn, uWSGI) поддерживают только WSGI
    - некоторые старые библиотеки или middleware могут работать только в синхронном режиме и не поддерживают ASGI
    ...
    - если используете только HTTP-запросы (API, стандартные страницы, ...), достаточно WSGI
  + runserver поддерживает только WSGI
* ASGI, Asynchronous Server Gateway Interface
  ...
  + поддерживает синхронные HTTP-запросы
  + поддерживает асинхронные протоколы (WebSocket, Server-Sent Events, ...)
  + ASGI-сервер (Daphne, Uvicorn, ...) обрабатывает синхронные HTTP-запросы и асинхронные соединения (WebSocket)
  + Django работает с библиотекой Django Channels, чтобы обрабатывать WebSocket
  + Подходит для приложений, работающих в реальном времени (чат, уведомления, игры)
...
* Redis сервер фактически выполняет работу по хранению и маршрутизации данных
