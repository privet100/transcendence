...
1) JS на фронте читает инпут, проверяет с помощью `regex`
  ...
  - Бэк ещё раз валидирует** (проверяет пароль и почту), так как фронтенд можно обойти или модифицировать
2) Первый запрос:
  - `get_token` (JS) запрашивает CSRF-токен с эндпоинта `api/csrf-token/`
  - CSRF-токен отдаётся при рендере HTML-страницы **или** через API-эндпоинт  
  - Если токен существует в cookie/сессии) — Django возвращает его  
  - Если нет — генерирует новый, сохраняет в сессии/cookie, возвращает  
  - для этого нужен `CsrfViewMiddleware`.  
  - для SPA отдельный эндпоинт — хорошее решение
  - Создание токена Django:
...
4) django отдаёт клиенту токен
  - Либо устанавливает cookie `csrftoken` в HTTP-ответе (через `@ensure_csrf_cookie`)
  - Либо в формате JSON — если SPA должен явно его прочитать
  - Либо в скрытом поле формы — для классических HTML-форм
...
5) второй запрос клиента   
  - JS обращается к REST API endpoints (`/history`, `/users/`, `/send`)
  - CSRF-токен добавляется:  
    + В заголовке `X-CSRFToken` для `POST`, `PUT`, `PATCH`, `DELETE` запросов
    + Для классических HTML-форм — токен в скрытом поле формы (только для POST)  
  - GET-запросы не требуют CSRF-защиты
6) `CsrfViewMiddleware`
  - Сравнивает токен из заголовка/тела формы с токеном из cookie (или сессии, если session-based CSRF)
  - Django использует сравнение в постоянное время (constant-time comparison) для предотвращения атак по времени
  - убедиться, что запрос пришёл от авторизованного пользователя, а не с другого сайта
  - предотвращает атаки от сайтов с другим `Origin`
7) декоратор `@csrf_protect`  
  - Используется, если глобальная CSRF-защита (`CsrfViewMiddleware`) не действует на конкретное представление  
  - Проверяет:
    + Наличие CSRF-токена в заголовке `X-CSRFToken` или в POST-данных  
    + Совпадение токена с сохранённым для текущего сеанса
  - гарантирует, что все изменяющие запросы (`POST`, `PUT`, `PATCH`, `DELETE`) содержат действительный CSRF-токен
8) `views.py` — дополнительные проверки (помимо CSRF, django проверяет авторизацию пользователя, валидацию данных, др проверки бизнес-логики)
9) Токен остаётся в cookie до ротации, API не обязан возвращать токен в каждом ответе, если клиент его уже имеет
10) после логина в другой вкладке или при нажатии "Назад"
  - CSRF-токен ротацируется после логина**, чтобы избежать атак с фиксацией сессии
  - При обновлении страницы форма загружается с новым токеном
