...
* webhook — механизм, который позволяет сторонним сервисам или вашим собственным вспомогательным сервисам отправлять уведомления (HTTP-запросы) на ваш сервер, когда у них что-то произошло
  + Интеграция с внешними сервисами. Если вам нужно получать уведомления от сторонней платформы (например, OAuth-сервис, платёжная система, почтовая рассылка, Slack и пр.), то при определённом событии (подтвердился платёж, пришло важное сообщение, пользователь подтвердил почту и т.п.) этот сервис будет «стучаться» на заранее заданный URL (webhook) вашего приложения. Таким образом, ваше приложение сразу узнаёт, что событие произошло, и может обработать его в реальном времени (обновить базу данных, отправить уведомление пользователю и т.д.).
  + Реакция на внутренние события. Иногда webhook используется не только для внешних сервисов, но и для микросервисов внутри вашего собственного проекта. Если архитектура Transcendence разбита на несколько сервисов, один сервис может вызвать webhook другого, чтобы сообщить об изменении статуса игрока, старте матча, сообщении в чате и т.д.
  + Уведомления о событиях без постоянного опроса. Вместо того чтобы фронтенд или какой-то сторонний сервис постоянно «дергал» ваш сервер (pull-модель), webhook работает по push-модели. Как только возникает событие, «инициатор» посылает запрос на ваш endpoint. Это уменьшает нагрузку и обеспечивает более оперативное обновление данных.
  + Асимметричная связь. Webhooks часто полезны, когда ваше приложение не может напрямую (или не хочет) постоянно подключаться к внешнему API и проверять статус. Вместо этого внешний API «сам придёт» к вам, когда что-то произойдёт (например, событие в социальной сети, оплата, результат матча в реальном времени).
  + Где это может применяться в Transcendence:
OAuth (42 API) или авторизация через сторонние платформы
При авторизации через 42-интра или GitHub/Google (если расширяете), некоторые из этих сервисов могут предоставлять webhooks или callback, куда они шлют подтверждения.

Чаты / мгновенные уведомления
Если используете микросервисную архитектуру (к примеру, реализуете чат на отдельном процессе с WebSocket), webhook может сигнализировать основному приложению о сообщении, которое пришло в чат, или об изменении статуса пользователя в реальном времени.

Интеграция со Slack, Discord или Telegram
Если проекту нужно отправлять уведомления о событиях (создана новая игровая лобби, кто-то подключился к матчу и т.д.) в командный чат в Slack/Discord/Telegram — это может быть реализовано через webhook.

Онлайн-игра
В игре (например, Pong), если есть внутренняя система подсчёта очков и ведение таблицы лидеров, а внешняя система должна «знать», когда кто-то выиграл матч, можно отправлять соответствующий webhook-запрос.

В Transcendence webhook чаще всего нужен для:

Получения уведомлений от внешних систем (OAuth, почта, платёжные сервисы и т.д.).
В некоторых случаях — для обмена событиями между вашими же микросервисами.
Организации архитектуры по принципу «push-уведомления», а не постоянного опроса.
Таким образом, webhook — это просто удобный способ «узнавать» о важных событиях в режиме реального времени, не перегружая ваш сервер и внешние сервисы лишними запросами.
...
* frontend/nginx.conf
  + конфигурация для веб-приложения с поддержкой статических файлов, защищённым соединением и обработкой вебхуков
