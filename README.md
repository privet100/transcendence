...
* 2. Первый запрос   
  + django.middleware.csrf.get_token запрашивает CSRF-токен с `api/csrf-token/` или **при рендере HTML-страницы**
  + dajngo генерирует токен
    - либо через CsrfViewMiddleware (автоматически при защищённых методах (POST, PUT, DELETE) при включённой CSRF-защите)
    - либо при использовании @ensure_csrf_cookie
    - либо при вызове django.middleware.csrf.get_token(request)
    - либо не генерирует, а возвращает уже существущий в cookie/сессии
* 3. django отдаёт клиенту токен
  + Либо устанавливает cookie `csrftoken` в HTTP-ответе (через **`@ensure_csrf_cookie`**)
    ...
    + **в SPA-приложениях не всегда так**
  ...
  🛠️ в SPA часто токен берётся из cookie, если он не HttpOnly, чтобы JS мог добавить его в заголовок.
  📌 Если токен отдается JSON-ответом, то JS вручную сохраняет его в cookie или локальном хранилище (но предпочтительнее использовать cookie)

* 4. второй запрос клиента: js обращается к endpoints `/profile`, `/chat/`, ...
...
* 5. `CsrfViewMiddleware` глобальная CSRF-защита
  - Сравнивает токен из заголовка/тела формы с токеном из cookie/сессии
  - проверяет совпадение токена из запроса и cookie, чтобы убедиться, что запрос инициирован тем же сайтом
  ...
  - если `CsrfViewMiddleware` тут не действует, ту же самую работу может проделать декоратор `@csrf_protect`
  
  ✍️ при использовании @csrf_exempt на уровне представления CSRF-проверка пропускается.
  💡Referer проверяется для запросов по HTTPS, чтобы избежать атак через смешанный контент

* 6. `views.py` дополнительные проверки (авторизация, валидацию данных, др)
  + не имеет отношения к CSRF
