...
* 2. Первый запрос:
  + `get_token` (JS) запрашивает CSRF-токен с `api/csrf-token/`
  + CSRF-токен отдаётся **при рендере HTML-страницы** или через API-эндпоинт  
  + Если токен существует в cookie/сессии — `CsrfViewMiddleware` возвращает его  
  + Если нет — `CsrfViewMiddleware` генерирует новый
* 3. django отдаёт клиенту токен
  + Либо устанавливает cookie `csrftoken` в HTTP-ответе (через `@ensure_csrf_cookie`)
    ...
    - токен остаётся в cookie до ротации
    - ротация (форма загружается с новым токеном) после логина в другой вкладке, при нажатии "Назад", при обновлении страницы
    + **в SPA-приложениях не всегда автоматически через куки**
...
* 6. декоратор `@csrf_protect` проверяет CSRF-токен в заголовке `X-CSRFToken` или в POST-данных (совпадение токена с сохранённым для текущего сеанса)
  - если глобальная CSRF-защита (`CsrfViewMiddleware`) не действует тут 
