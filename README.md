### tour
* Создание турнира
  + `TourPage.js: `createTour()`
    - отправляет POST-запрос на сервер для создания турнира через `/api/tour/`
    -  запросе отправляются данные о турнире, такие как имя турнира, время начала и игроки.
  + На сервере, в `TourView`, обработчик `post` получает запрос и создает новый турнир через сериализатор `TourSerializer`. После создания турнира вызывается метод `create_games()` для создания матчей (каждый с каждым).

2. **Приглашение игроков (models.py)**
    - После создания турнира, метод `invite_players()` отправляет сообщения с приглашением игрокам через `ChatMessage`. Это сообщение содержит приглашение в турнир и запрос на подтверждение участия.
    - Также, после истечения времени (5 минут), если количество игроков больше 1, создаются игры для турнира через метод `create_games()`. Если игроков недостаточно, турнир отменяется.

3. **Запуск турнира и создание матчей (models.py)**
    - В методе `create_games()`, для каждого игрока создается матч с каждым другим игроком турнира. Это создаются все возможные пары игроков для игр.

### Описание кода по частям:

#### 1. **`TourPage.js` (Frontend)**
   - **Конструктор**: В конструкторе создается турнир и загружаются данные профиля пользователя и рейтингов.
   - **Метод `createTour()`**: Отправляет запрос на сервер для создания турнира. Если турнир успешно создан, происходит редирект на страницу турнира.
   
#### 2. **`views.py` (Backend)**
   - **Метод `get()`**: Позволяет получить информацию о турнире по его ID или список всех турниров.
   - **Метод `post()`**: Создает новый турнир и вызывает метод для создания игр (`create_games()`). Также можно отправить сообщение о начале турнира.
   - **Метод `post_invitation_response()`**: Обрабатывает ответ на приглашение. Если игрок согласен, он добавляется в турнир.

#### 3. **`models.py` (Backend)**
   - **`Tour`**: Модель турнира с полями для имени, игроков, игр, времени начала и состояния турнира.
     - **Метод `invite_players()`**: Приглашает игроков через сообщения и устанавливает время начала турнира.
     - **Метод `create_games()`**: Создает игры между всеми участниками (каждый с каждым) и добавляет их в турнир.

### Ошибки и улучшения:

1. **Ошибка в методе `createTour()` на фронтенде**:
   В строке:
   ```javascript
   ths.tour = {
   ```
   ...
   this.tour = {
   ```
   Вместо `ths` должно быть `this`. Это ошибка синтаксиса, так как в JavaScript `this` указывает на текущий объект, а `ths` — это опечатка.

2. **Ошибка в `TourView`**: У вас дважды определены методы `post()`:
   - Один создает турнир и вызывает метод для создания игр.
   - Второй отвечает за создание турнира и отправку сообщений пользователям.
   
   Нужно удалить один из методов `post()`, чтобы избежать дублирования. Например, метод, который отправляет сообщения через WebSocket, можно вызвать в другом месте.

3. **Ошибка в `models.py` в методе `invite_players()`**:
   В методе `invite_players()` есть ошибка в строке:
   ```python
   ChatMessage.objects.create(
       sender=self.players.first(),
       receiver=user,
       text=f"Турнир '{self.name}' начинается, перейди по ссылке /tournament/{self.id}"
   )
   ```
   Нужно добавить проверку на то, что объект `user` существует, потому что вы используете его в цикле, но сам объект пользователя не определен. Также стоит проверить, что переменная `self.players.first()` содержит правильного игрока, который будет отправлять сообщение.

4. **Отсутствие механизма учета времени**:
...
5. **Обработка ошибок**:
   Ваш код не обрабатывает случай, когда турнир не может быть создан или нет достаточного количества игроков. Нужно добавить дополнительные проверки и возвращать соответствующие сообщения об ошибке.

### Резюме работы турнира:

1. **Создание турнира**: Пользователь создает турнир через фронтенд, отправляя запрос на сервер. На сервере создается объект турнира.
2. **Приглашение игроков**: После создания турнира, игрокам отправляются приглашения участвовать в турнире через сообщения. Игроки могут ответить "да" или "нет".
3. **Создание матчей**: Если количество игроков достаточно, создаются игры между участниками (каждый с каждым).
4. **Запуск турнира**: Когда все игры созданы, турнир может быть запущен.

Ваш проект требует нескольких улучшений, таких как исправление синтаксических ошибок и улучшение обработки времени и ошибок, чтобы обеспечить правильную работу турниров.
...
* 2) Установка защищённого соединения (TLS-рукопожатие)
  ...
  + nginx генерирует идентификатор сессии TLS **session id** (если новое соединение, для возобновления сеанса в рамках одной TCP-сессии)
    - сохраняет  
    - отправляет браузеру   
  ...
  + браузер генерирует **pre-master secret** (предварительный ключ для симметричного шифрования)  
    - генерирует **сессионный ключ** (симметричный ключ, symmetric keys, derived key) на его основе (для шифрования последующего трафика)
    - шифрует с использованием .crt, отправляет серверу, не сохраняет
  + nginx расшифровывает pre-master secret с помощью`.key и .crt     
    - генерирует **сессионный ключ** на его основе (для шифрования последующего трафика)
    - не сохраняет
