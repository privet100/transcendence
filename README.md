...
* настройка Django Channels для поддержки системы каналов 
  ...
  + бэкэнд для обмена сообщениями: Redis
...
* получил данные - публикует в канал, доставляет подписанным
...
* доставлка соообщений на другой сервер: бэкэнд, брокер сообщений, посредник
...
* можно вместе reids использовать для этого: PostgreSQL, RabbitMQ
...
* для матчей в реальном времени (текущее состояние игры, обмен данными между игроками, хранение лидербордов с помощью sorted sets)
  + база данных
...
* настройте Redis
  ...
  + **защищён паролем** и доступен только внутри сети Docker, чтобы предотвратить несанкционированный доступ
...
* убедитесь, что Redis работает: `redis-cli ping`, Ожидаемый ответ: `PONG`
