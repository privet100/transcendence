...
* в SPA-приложениях
  + CSRF-токен не всегда автоматически передаётся через куки (например, если `HttpOnly` установлен)  
  + CSRF-токен часто нужно отправлять явно, так как `HttpOnly` делает куки недоступными для JS
  + обычно фронт получает токен при загрузке страницы и добавляет его в заголовок запросов.
* фронтенд может проверять форму (например, обязательные поля) - это не связано с CSRF
...
2) js обращается к rest api endpoints /history, /users/, /send (post) 
3) первый запрос: js запрашивает CSRF-токен с эндпоинта `api/csrf-token/`
4) django создаёт токен (заново для каждой сессии)
5) django сохраняет токен в cookie `csrftoken` 
  + с клиентской стороны только для чтения (не `HttpOnly`)
  + не доступен для сторонних доменов (`SameSite=Lax` или `SameSite=Strict`)
  + `Secure`
6) django отдаёт клиенту токен в формате JSON
  + **если он не передаётся автоматически через куки (?)**
7) js сохраняет токен в cookie `document.cookie = csrftoken=...` (django его там сохраняет или js?)
