...
* validation for forms and any user input
  + кроме формы на странице входа: CSRF-токен не нужен на странице входа, пользователь ещё не аутентифицирован, CSRF-атака не имеет смысла
    - исключение: если логин-запрос меняет состояние сервера (автоматическая регистрация, ...), можно использовать CSRF для дополнительной защиты
  + CSRF актуален когда браузер уже автоматически отправляет cookies с сессионным идентификатором
* защищает взаимодействие UI-приложения с бэкендом через HTTP-запросы
  + особенно важно, если SPA обращается к API, использующему сессионные куки
* защищает от атак подделки межсайтовых запросов, от подделки запроса от пользователя через его браузер (CSRF-атак):
  + предотвращает атаки от сайтов с другим `Origin`
  + пользователь вошел в аккаунт на сайте, а затем перешел на вредоносный сайт, вредоносный сайт отправляет запрос от имени пользователя
  + актуально для HTTP-запросов, особенно `POST`, `PUT`, `DELETE`: эти методы меняют состояние сервера, поэтому требуют защиты
  + но поддомены — отдельная тема
    - для защиты между поддоменами лучше использовать CORS (Cross-Origin Resource Sharing)
    - a POST request from subdomain.example.com from succeeding against api.example.com
* при работе с ws не используется
* в stateful-приложениях, где sessionid или cookie для отслеживания пользователя 
   +когда клиент аутентифицируется через cookie, автоматически отправляемые браузером
* в stateless-приложениях с JWT-авторизацией не используется
  + если JWT хранится в `localStorage` или `sessionStorage` и явно отправляется в заголовке `Authorization`
  + но если JWT хранится в cookie, CSRF может понадобиться, если cookie не `SameSite=Strict`
* в SPA-приложениях не всегда автоматически передаётся через куки
  + например, если `HttpOnly` установлен, `HttpOnly` делает куки недоступными для JS 
  + CSRF-токен нужно отправлять явно
  + фронт получает токен при загрузке страницы
* должен проверяться только на сервере, так как фронтенд можно подделать
  + **в Django через `@csrf_protect` или `CsrfViewMiddleware`**
* бэк ещё раз валидирует (проверяет пароль и почту, ...)
  + фронтенд можно обойти или модифицировать
  + только сервер должен принимать окончательные решения о валидности данных  
  + защита от CSRF — это отдельная проверка, которая подтверждает подлинность запроса

1) js на фронте читает инпут, проверяет с помощью **regex**
  + валидация данных (например, проверка e-mail, пароля)
  + это не имеет отношения к CSRF  
2) первый запрос: `get_token` (js) запрашивает CSRF-токен с эндпоинта `api/csrf-token/`
  + CSRF-токен отдаётся при рендере HTML-страницы
  + если токен, привязанный к текущему сеансу/запросу существует (в cookie, в памяти сессии, ..), то возвращает его  
  + если нет, то генерирует новый, сохраняет его в сессии (или cookie) и возвращает
  + для этого нужен `CsrfViewMiddleware`
  + это типичный подход в SPA-приложениях, где формы создаются динамически и нужно получать CSRF-токен с бэкенда
    - для SPA отдельный эндпоинт — хорошее решение
  + django создаёт CSRF-токен при первой необходимости
    - не обязательно новый для каждой сессии  
    - токен может быть привязан к сессии
    - токен обновляется при логине, чтобы избежать атак с захваченными токенами
3) django сохраняет токен в cookie `csrftoken` 
  + только для чтения клиентом (не `HttpOnly`, чтобы JS мог прочитать при SPA)  
  + sameSite=Lax или Strict - защищает от межсайтовых запросов
  + secure = передаётся только по HTTPS  
4) django отдаёт клиенту токен:
  + либо сам устанавливает cookie `csrftoken` в HTTP-ответе, если используется `@ensure_csrf_cookie` 
  + либо в ответе в формате JSON
    - если SPA должен явно его прочитать (например, если используете API-запрос без предварительной загрузки HTML)
  + либо в скрытом поле формы
  + js может прочитать токен из cookie
5) второй запрос клиента
  + js обращается к rest api endpoints /history, /users/, /send (post) 
  + токен в заголовке post/put/delete-запросов
  + токен в скрытом поле формы при post/get запросах
  + токен в заголовке `X-CSRFToken` при отправке POST-запросов 
  + для защищённых методов (`POST`, `PUT`, `DELETE`) нужно добавить CSRF-токен в заголовок
  + возвращается серверу с каждым POST-запросом, чтобы сервер мог проверить, что запрос действительно был отправлен с доверенной страницы
9) csrf middleware проверяет токен
10) django сравнивает токен из запроса с токеном из cookie
  + чтобы знать, что это не юзер с третьего сайта
11) views.py **набор проверок**
12) передаётся в ответах сервера
  + если есть текущий CSRF-токен, привязанный к пользователю (**сессии или куки**), возвращает его  
13) after logging in in another browser tab or hitting the back button after a login, you reload the page with the form (the token is rotated after a login)



### ✅ **8) Второй запрос клиента:**  
✔️ Всё верно!  
- 🔐 **Токен в заголовке** для API-запросов:  
  ```javascript
  fetch('/api/send/', {
      method: 'POST',
      headers: {
  ...
  ```
- 📝 **Токен в скрытом поле формы** — если форма отправляется без JS.  
- 🚫 **GET-запросы CSRF-токена не требуют**, так как они не изменяют данные.  

---

### ✅ **9) CSRF middleware проверяет токен**  
✔️ Да, `CsrfViewMiddleware` выполняет эту проверку автоматически.  

---

### ✅ **10) Django сравнивает токен из запроса с токеном из cookie**  
✔️ Именно так.  
💡 **Если токены не совпадают — 403 Forbidden.**  

---

### ✅ **11) `views.py` — набор проверок**  
✔️ Помимо CSRF Django проверяет другие вещи (например, авторизацию, валидацию данных).  

---

### ⚡ **12) Передаётся в ответах сервера:**  
🔷 **Уточнение:**  
- Если у пользователя есть токен, он остаётся в cookie.  
- **Обычно CSRF-токен не "возвращается" в каждом ответе**, если только специально не запросить.  

---

### ✅ **13) После логина в другой вкладке или при нажатии "Назад" страница обновляется, так как токен ротацируется**  
✔️ Абсолютно верно!  
Django **ротацирует CSRF-токен после логина**, чтобы предотвратить фиксацию сессии.  

---

### ✍️ **Итоговые правки плана:**  
1. **JS проверяет данные формы (не связано с CSRF).**  
2. **Первичный запрос может быть к `api/csrf-token/`, если SPA.**  
3. **Django устанавливает `csrftoken` в cookie (JS не должен делать это вручную).**  
4. **Для API-запросов JS добавляет `X-CSRFToken` в заголовок.**  
5. **CSRF middleware сравнивает токен из заголовка с токеном из cookie.**  
6. **После логина токен обновляется (ротация).**  
...
* CSRF_TRUSTED_ORIGINS = trusted origins for **unsafe** requests (e.g. POST)
  ...
  + вхожу в /admin: **Forbidden (403). CSRF verification failed.** => https://localhost:4443 does not match any trusted origins
