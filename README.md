...
* myproject/
  ...
  + `settings.py` 
    ...
    - `ASGI_APPLICATION` = "myproject.asgi.application": Django + Channels используют ASGI для работы с WebSocket и др асинхр протоколами
    - `WSGI_APPLICATION` точка входа для WSGI-серверов, если запускаете проект в синхронном режиме (или для **совместимости**)
    - `ASGI_APPLICATION` + `WSGI_APPLICATION` => проект может работать и под WSGI, и под ASGI
...
* Зачем нужен WSGI?
  + WSGI всё ещё нужен в некоторых случаях
    - особенно если вы работаете с инструментами или средами, которые изначально поддерживают только синхронные HTTP-запросы
