...
* myproject/
  ...
  + `settings.py` 
    ...
    - как Django ведёт себя на высоком уровне (middleware, установленными приложения, роутинг, шаблоны, локаль)  
    - Использует среды (ENV-переменные) и пакеты типа `decouple` и `dotenv`, чтобы секретные данные (пароли, **ключи OAuth**) не хранились в коде
    ...
    - `INSTALLED_APPS` список приложений (apps), которые активны в проекте: стандартные (admin, auth, sessions и кастомные: `myapp`, `chat`, `api_42`, `auth_app`, ...
...
* django:
  ...
  + Models описывают структуру данных и связи между ними (пользователи, профили, чаты, сообщения, статистика игры и пр.)
  + Views обрабатывают логику запросов (создание комнаты, отправка сообщения, обновление профиля
    - определяет функция для обработки endpoint
  + URLs: какие функции обрабатывают какие пути
...
* «два сервиса» = два отдельных процесса
  + нужен один процесс NGIN
* «два разных сервера» с точки зрения NGINX = два server{}-блока
  + в конфигурации — столько server{}-блоков, сколько нужно
...
* `models.py`  
* `views.py` функции при обраении под адресу = эндпоинт
* `admin.py`
