...
* `Daphne` не обслуживает статические файлы по умолчанию, CSS не загружается
  + может обрабатывать статические файлы без внешнего сервера: добавьте библиотеку WhiteNoise
* `runserver` Django самостоятельно обслуживает статические файлы
* обычно через Nginx
  + эффективно
  + настроить nginx
  ...
  + пути к статическим файлам в `nginx.conf` и `settings.py` совпадают
    ...
    - `nginx.conf`:
      ...
      location /staticfiles/ {
          alias /app/staticfiles/; # реальная директория внутри **контейнера Nginx**, куда Docker копирует `staticfiles`
      }
      ```
...
* проверка 
  + `https://localhost:4443/staticfiles/base.css` (Статические файлы обслуживаются чер Daphne)
  ...
  + `docker exec -it <frontend_container> nginx -t` проверьте корректность конфигурации Nginx перед перезапуском
  + `docker-compose logs frontend`
  + `docker-compose logs backend`
  + `http://localhost:8000/static/css/style.css` публичный, стат файлы доступны на сайте (`STATIC_URL = '/static/'`)
